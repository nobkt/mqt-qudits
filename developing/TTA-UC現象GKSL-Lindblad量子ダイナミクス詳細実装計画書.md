...
class GKSLPhysicalParameters:
    ...
    def validate(self) -> None:
        ...
        if self.E_T <= 0:
            raise ValueError(
                f"三重項エネルギーE_Tは正でなければなりません: E_T={self.E_T}"
            )
        if self.E_S <= 0:
            raise ValueError(
                f"一重項エネルギーE_Sは正でなければなりません: E_S={self.E_S}"
            )
        if self.E_S <= self.E_T:
            raise ValueError(
                f"一重項エネルギーは三重項より大きくなければなりません: E_S={self.E_S}, E_T={self.E_T}"
            )
        ...
        if energy_ratio > 0.1:
            raise ValueError(
                f"エネルギー関係式 2*E_T ≈ E_S が満たされません: 2*E_T={2*self.E_T}, E_S={self.E_S}, 誤差={energy_ratio*100:.1f}%"
            )
        ...
        for name, rate in rates.items():
            if rate < 0:
                raise ValueError(
                    f"散逸レート{name}は非負でなければなりません: {name}={rate}"
                )
        ...
        if not (self.gamma_TTA >= self.Gamma_fl):
            raise ValueError(
                f"時間スケール階層違反: gamma_TTA >= Gamma_fl が必要 (gamma_TTA={self.gamma_TTA}, Gamma_fl={self.Gamma_fl})"
            )
        if not (self.Gamma_fl >= self.k_IC):
            raise ValueError(
                f"時間スケール階層違反: Gamma_fl >= k_IC が必要 (Gamma_fl={self.Gamma_fl}, k_IC={self.k_IC})"
            )
        if not (self.k_IC >= self.k_ISC_ST):
            raise ValueError(
                f"時間スケール階層違反: k_IC >= k_ISC_ST が必要 (k_IC={self.k_IC}, k_ISC_ST={self.k_ISC_ST})"
            )

        # 5. システムパラメータの妥当性
        if self.N_molecules < 2:
            raise ValueError(
                f"分子数N_moleculesは2以上でなければなりません: N_molecules={self.N_molecules}"
            )
        if self.d != 3:
        ...
        if self.with_boson:
            if self.n_max < 1:
                raise ValueError(
                    f"フォノンカットオフn_maxは1以上でなければなりません: n_max={self.n_max}"
                )
            if self.omega_ph <= 0:
                raise ValueError(
                    f"フォノン周波数omega_phは正でなければなりません: omega_ph={self.omega_ph}"
                )
            if self.g_eph < 0:
                raise ValueError(
                    f"電子-フォノン結合g_ephは非負でなければなりません: g_eph={self.g_eph}"
                )
        ...
        if max_dissipation > 0.5 * min_energy:
            raise ValueError(
                f"弱結合近似違反: 最大散逸レート({max_dissipation})がシステムエネルギー({min_energy})に対して大きすぎます"
            )
...
def build_lindblad_operators(params: GKSLPhysicalParameters) -> list:
    ...
    for i in range(N - 1):
        ...
        lindblad_ops.append(
            (np.sqrt(params.gamma_TTA / 2) * L_TTA_1, params.gamma_TTA / 2)
        )
        ...
        lindblad_ops.append(
            (np.sqrt(params.gamma_TTA / 2) * L_TTA_2, params.gamma_TTA / 2)
        )
    ...
    if len(lindblad_ops) != 26:
        raise ValueError(
            f"Lindblad演算子の数が不正です: {len(lindblad_ops)} (期待値: 26)"
        )
...
def stinespring_unitary_from_lindblad(L: np.ndarray, dt: float) -> np.ndarray:
    ...
    # 生成子G（反Hermitian行列）
    G = np.block([[np.zeros_like(L), L], [L.conj().T, np.zeros_like(L)]])
...
def build_trotter_step_classical(
    H_0: np.ndarray, H_transfer: np.ndarray, lindblad_ops: list, dt: float
) -> callable:
    """
    ...
    for L, gamma in lindblad_ops:
        ...
        # 超演算子形式
        L_D += np.kron(L, L.conj()) - 0.5 * (
            np.kron(L_dag_L, I) + np.kron(I, L_dag_L.T)
        )
...
def validate_density_matrix(
    rho: np.ndarray, step: int = 0, tolerance: dict = None
) -> dict:
    """
    密度行列の整合性を検証
    ...
    if abs(trace_val - 1.0) > tolerance["trace"]:
        errors.append(
            f"トレース保存違反: Tr[ρ]={trace_val} (許容誤差: {tolerance['trace']})"
        )
...
def validate_particle_conservation(
    populations: dict, N_molecules: int, tolerance: float = 1e-8
) -> bool:
    """
    粒子数保存則を検証
...
def validate_entropy_increase(
    S_prev: float, S_curr: float, tolerance: float = -1e-8
) -> bool:
    """
    エントロピー増大則を検証（第2法則）
...
class ClassicalGKSLSimulator:
    ...
    def simulate(
        self, t_max: float, n_steps: int, initial_state: str = "edge_triplet"
    ) -> dict:
        """
        時間発展シミュレーションを実行
...
class QuditGKSLSimulator:
    ...
    def build_lindblad_circuit_single(
        self, L: np.ndarray, dt: float, ancilla_idx: int
    ) -> QuantumCircuit:
        """
        ...
        if d_stinespring == 6:
            ...
            circuit = QuantumCircuit(
                self.n_system_qudits + 1, radii=[3] * self.n_system_qudits + [2]
            )
            ...
            circuit.custom_unitary(
                U_stinespring,
                [target_qudits[0], target_qudits[1], self.n_system_qudits],
            )
        else:
            raise RuntimeError(
    ...
    def build_trotter_step_circuit(self, dt: float) -> QuantumCircuit:
        ...
        circuit = QuantumCircuit(
            total_qudits, radii=[3] * self.n_system_qudits + [2] * self.n_ancilla_qubits
        )
        ...
        for idx, (L, gamma) in enumerate(self.lindblad_ops):
            L_circuit = self.build_lindblad_circuit_single(
                L, dt, self.n_system_qudits + idx
            )
...
def check_forbidden_states(rho_qubit, N=4):
    ...
    if P_forbidden > 1e-8:
        raise PhysicsViolationError(f"Forbidden state leakage: {P_forbidden}")
```
...
def build_H_total_boson(params) -> np.ndarray:
    ...
    # エルミート検証
    assert (
        np.linalg.norm(H_total - H_total.conj().T) < 1e-10
...
class ClassicalGKSLBosonSimulator:
    ...
    def simulate(
        ...
        rho_final_total = sol.y[:, -1].reshape(
            (self.dim_total, self.dim_total), order="F"
        )
...
class QubitGKSLBosonSimulator:
    ...
    def __init__(self, params: GKSLPhysicalParameters):
        ...
        self.n_sys_qubits = (
            2 * params.N_molecules + self.n_phonon_qubits_per_mol * params.N_molecules
        )
        self.n_ancilla = 26
...
class TestBosonSimulators:
    ...
    def test_classical_boson_initialization(self):
        """古典ボソンシミュレータの初期化"""
        params = GKSLPhysicalParameters(
            with_boson=True, n_max=2, omega_ph=0.15, g_eph=0.02
        )
    ...
    def test_zero_coupling_limit(self):
        ...
        params_b = GKSLPhysicalParameters(
            with_boson=True, n_max=2, omega_ph=0.15, g_eph=0.0
        )
...
class TestIntegration:
    ...
    def test_unitary_limit(self):
        ...
        params = GKSLPhysicalParameters(
            gamma_TTA=0, Gamma_fl=0, Gamma_ph=0, k_IC=0, k_ISC_ST=0, k_ISC_TS=0
        )
    ...
    def test_fluorescence_analytical(self):
        ...
        params = GKSLPhysicalParameters(
            V=0, gamma_TTA=0, Gamma_fl=0.01, Gamma_ph=0, k_IC=0, k_ISC_ST=0, k_ISC_TS=0
        )
        # 初期状態: 全分子S1
        sim = ClassicalGKSLSimulator(params)
...
def plot_gksl_comparison(
    result_unitary: dict, result_gksl: dict, title: str = None, save_path: str = None
):
    """
...
class QuditGKSLNoisySimulator(QuditGKSLSimulator):
    ...
    def apply_noise(self, rho: np.ndarray, gate_type: str) -> np.ndarray:
        ...
        if gate_type == "2-qudit":
            d = rho.shape[0]
            rho_noisy = (1 - self.p_depol) * rho + self.p_depol / (d**2 - 1) * (
                d * np.eye(d) * np.trace(rho) - rho
            )
            return rho_noisy
        else:
    ...
    def simulate(
        self, T_total, N_steps, initial_state_type, use_shots=False, n_shots=10000
    ) -> dict:
        """ハードウェアノイズ付きGKSLシミュレーション"""
        ...
```
...
- [x] 蛍光のみのテストで解析解と一致（$10^{-3}$）（PR#147で実装：V=0パラメータ + 'all*singlet'初期状態 → $N*{S*1}(t) = 4 e^{-\Gamma*{\text{fl}} t}$ と一致）
