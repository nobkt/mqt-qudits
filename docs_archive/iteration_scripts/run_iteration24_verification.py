#!/usr/bin/env python3
"""Iteration 24 verification script.

Checks:
- Issue DD fix: QuditGKSLCircuitBosonSimulator.rho_final now returns
  electronic-only reduced density matrix (partial trace over phonons),
  consistent with QuditGKSLBosonSimulator.
- Issue EE fix: Circuit boson simulator result dict keys now match
  DM boson simulator (n_system_qudits, n_phonon_qudits, n_total_qudits,
  estimated_gates_per_step).
- Cross-simulator consistency: DM and circuit boson simulators produce
  matching rho_final (within numerical precision).
- Regression: All iteration 23 checks (62 checks) still pass.

Usage:
    cd tutorials
    python run_iteration24_verification.py
"""
...
def main() -> None:
    ...
    # ================================================================
    # Section 1-10: Iteration 23 regression checks (62 checks)
    # ================================================================
    import numpy as np
    ...
    # ================================================================
    # Section 11: Issue DD - Circuit boson rho_final consistency (NEW)
    # ================================================================
    ...
    # rho_final should be electronic-only (dim_el x dim_el), not full extended
    dim_el = params_boson.d ** params_boson.N_molecules  # 3^2 = 9
    ...
    # rho_final should match DM simulator's rho_final
    ...
    # ================================================================
    # Section 12: Issue EE - Circuit boson result dict keys (NEW)
    # ================================================================
    print("\n=== Section 12: Circuit boson result dict key consistency ===")

    # Check that circuit boson result dict has same keys as DM boson
    add(
    ...
    # Values should match DM simulator
    add(
    ...
    # Old keys should NOT exist (renamed)
    add(
    ...
    out_file = os.path.join(
        out_dir, f"iteration24_circuit_boson_consistency_{results['timestamp']}.json"
    )
