#!/usr/bin/env python3
"""TTA-UC GKSL iteration 22 verification: Corrected error decomposition with SCPT.

Changes from iteration 21:
  - Test A-D: unchanged
  - Test E (modified):
    * Add "Symmetric Classical Product Trotter" (SCPT) comparison target:
      uses palindromic (symmetric) ordering of exact channel exponentials,
      matching the ST simulator's ordering structure.
    * New function _symmetric_classical_product_trotter_simulate()
    * ST vs SCPT cleanly isolates the Stinespring approximation error
      (iteration 21's ST vs CPT was flawed due to ordering mismatch)
    * SCPT vs Exact should show O(dt^2) convergence
    * Added signed N_S1 error, cancellation ratio analysis, polynomial fitting
    * 9 comparison pairs (added SCPT vs Exact, ST vs SCPT, SCPT vs CT)
    * Keep all iteration 21 pairs for backward compatibility
...
def test_exact_liouvillian_comparison(params: GKSLPhysicalParameters) -> dict:
    ...
    MODIFIED from iteration 21:
    - Add "Symmetric Classical Product Trotter" (SCPT) — palindromic ordering
      of exact channel exponentials, matching ST's ordering structure.
    - ST vs SCPT cleanly isolates Stinespring approximation error (fixes
      iteration 21's flawed ST vs CPT comparison).
    - 9 comparison pairs (added SCPT vs Exact, ST vs SCPT, SCPT vs CT).
    - Added signed N_S1 error, cancellation ratio, polynomial fitting.
    """
    print("\n" + "=" * 70)
    print("Test E: Exact Liouvillian Comparison (ST vs Exact vs CT vs CPT vs SCPT)")
    print("       [iter 22: +Symmetric CPT (SCPT), corrected error decomposition]")
    ...
    # --- Part 6: Polynomial fitting T(ST,exact) ≈ a*dt + b*dt² ---
    print(f"\n  Part 6: Polynomial fitting T(ST,exact) ≈ a·dt + b·dt²")
    ...
    if len(dts) >= 2:
        ...
        print(f"  Fit: T ≈ {a_fit:.6e} · dt + {b_fit:.6e} · dt²")
...
def main() -> int:
    ...
    print("=" * 70)
    print("TTA-UC GKSL Iteration 22: Corrected Error Decomposition with SCPT")
    ...
    results: dict = {
        "timestamp": timestamp,
        "iteration": 22,
        "purpose": "Corrected error decomposition: add Symmetric Classical Product Trotter "
                   "(SCPT) to cleanly isolate Stinespring approximation error (ST vs SCPT). "
                   "Iteration 21's ST vs CPT was flawed due to ordering mismatch. "
                   "SCPT matches ST's palindromic ordering with "
                   "exact channel exponentials.",
    ...
    # Write JSON report
    # ===================================================================
    json_path = output_dir / f"iteration22_scpt_decomposition_{timestamp}.json"
    ...
    # Test E summary (MODIFIED for iteration 22)
    md_lines.extend([
    ...
    if te.get("polyfit"):
        ...
        md_lines.extend([
            ...
            "",
            f"- a = {pf['a_coefficient']:.6e}（Stinespring 誤差係数）",
            f"- b = {pf['b_coefficient']:.6e}（Strang/残差 誤差係数）",
            f"- 相対フィット誤差: {', '.join(f'{e:.4f}' for e in pf['relative_fit_errors'])}",
            "",
        ])

    md_path = output_dir / f"iteration22_scpt_decomposition_{timestamp}.md"
    md_path.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
    ...
    if te.get("polyfit"):
        pf = te["polyfit"]
        print(f"  Test E: polyfit T ≈ {pf['a_coefficient']:.2e}·dt + {pf['b_coefficient']:.2e}·dt²")
