#!/usr/bin/env python3
"""TTA-UC GKSL iteration 23 verification: Improved error analysis with component-wise fitting.

Changes from iteration 22:
  - Test A-D: unchanged
  - Test E (improved analysis):
    * Add T(SCPT,CT) to main comparison table for completeness
    * Add SCPT-based cancellation ratio alongside CT-based
    * Add component-wise polynomial fitting:
      - T(ST,SCPT) = a_stine * dt  (pure Stinespring error, O(dt))
      - T(SCPT,exact) = b_strang * dt^2  (Strang+palindromic residual, O(dt^2))
    * Add normalized error coefficients: T/dt^n ratios
    * Iteration 22 aggregate fitting T(ST,ex) ~ a*dt + b*dt^2 retained as reference
    * (Iteration 22 confirmed SCPT implementation and all convergence rates)
...
def test_exact_liouvillian_comparison(params: GKSLPhysicalParameters) -> dict:
    ...
    MODIFIED from iteration 22:
    - Add T(SCPT,CT) to main comparison table
    - Add SCPT-based cancellation ratio alongside CT-based
    - Add component-wise polynomial fitting:
      * T(ST,SCPT) = a_stine * dt  (Stinespring error)
      * T(SCPT,ex) = b_strang * dt^2  (Strang+palindromic residual)
    - Add normalized error coefficients (T/dt^n ratios)
    - Retain iteration 22's aggregate fitting as reference
    """
    print("\n" + "=" * 70)
...
def main() -> int:
    ...
    print("=" * 70)
    print("TTA-UC GKSL Iteration 23: Improved Error Analysis with Component-wise Fitting")
    ...
    results: dict = {
        "timestamp": timestamp,
        "iteration": 23,
        "purpose": "Improved error analysis with component-wise fitting. "
                   "Iteration 22 confirmed SCPT and all convergence rates. "
                   "This iteration adds: (1) component-wise polynomial fits "
                   "(T(ST,SCPT)=a*dt, T(SCPT,ex)=b*dt^2), "
                   "(2) SCPT-based cancellation ratio, "
                   "(3) T(SCPT,CT) in main table, "
                   "(4) normalized error coefficients.",
    ...
    # Write JSON report
    # ===================================================================
    json_path = output_dir / f"iteration23_component_analysis_{timestamp}.json"
    ...
    # ===================================================================
    md_lines = [
        "# TTA-UC GKSL Iteration 23: 改善版誤差分析 — 成分別フィッティングと正規化係数",
        "",
        f"- 実行時刻(UTC): {timestamp}",
        "- iteration 22 からの主要修正:",
        "  - Test E: 成分別多項式フィッティングの追加",
        "    - T(ST,SCPT) = a_stine · dt（Stinespring 誤差）",
        "    - T(SCPT,ex) = b_strang · dt²（Strang+回文順残差）",
        "  - T(SCPT,CT) の主テーブルへの追加",
        "  - SCPT 基準の相殺比の追加",
        "  - 正規化誤差係数テーブルの追加",
        "  - 集約フィッティング T(ST,ex) ≈ a·dt + b·dt² は参考値として維持",
        "",
    ]
    ...
    # Test E summary (MODIFIED for iteration 23)
    md_lines.extend([
        "## Test E: 正確な GKSL Liouvillian 解との比較（iteration 23 改善版）",
        "",
    ...
    md_path = output_dir / f"iteration23_component_analysis_{timestamp}.md"
    md_path.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
