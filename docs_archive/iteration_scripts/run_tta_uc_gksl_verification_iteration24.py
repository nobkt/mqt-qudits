#!/usr/bin/env python3
"""TTA-UC GKSL iteration 24 verification: Asymptotic analysis with extended dt range.

Changes from iteration 23:
  - Test A-D: unchanged
  - Test E (extended analysis):
    * Extended dt range: n_steps = [5, 10, 20, 50, 100] (added dt=0.1)
    * Higher-order fitting: T = c0*dt^n + c1*dt^(n+1) for each component
    * Error vector angle estimation (trace distance law of cosines)
    * Richardson extrapolation for asymptotic coefficient estimation
    * Parts 6a-6c retained from iteration 23
    * (Iteration 23 confirmed component-wise fitting works well, <1.1% error)
...
def test_exact_liouvillian_comparison(params: GKSLPhysicalParameters) -> dict:
    ...
    MODIFIED from iteration 23:
    - Extended dt range: n_steps = [5, 10, 20, 50, 100] (dt=0.1 added)
    - Part 6b': Higher-order fitting T = c0*dt^n + c1*dt^(n+1)
    - Part 6d: Error vector angle estimation (trace distance law of cosines)
    - Part 6e: Richardson extrapolation for asymptotic coefficients
    - Parts 6a-6c retained from iteration 23
    """
    print("\n" + "=" * 70)
...
def main() -> int:
    ...
    print("=" * 70)
    print("TTA-UC GKSL Iteration 24: Asymptotic Analysis with Extended dt Range")
    ...
    results: dict = {
        "timestamp": timestamp,
        "iteration": 24,
        "purpose": "Asymptotic analysis with extended dt range. "
                   "Iteration 23 confirmed component-wise fitting (<1.1% error). "
                   "This iteration adds: (1) extended dt range (dt=0.1), "
                   "(2) higher-order fitting T=c0*dt^n+c1*dt^(n+1), "
                   "(3) error vector angle estimation, "
                   "(4) Richardson extrapolation for asymptotic coefficients.",
    ...
    # Write JSON report
    # ===================================================================
    json_path = output_dir / f"iteration24_asymptotic_analysis_{timestamp}.json"
    ...
    md_path = output_dir / f"iteration24_asymptotic_analysis_{timestamp}.md"
    md_path.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
