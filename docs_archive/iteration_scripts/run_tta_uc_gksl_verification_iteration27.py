#!/usr/bin/env python3
"""TTA-UC GKSL iteration 27 verification: Higher-order Frobenius fitting.

Changes from iteration 26:
  - Test A-D: unchanged
  - Test E (extended analysis):
    * Frobenius distance pairs EXPANDED: frob_CT_vs_exact, frob_SCPT_vs_CT added
    * Part 6f IMPROVED: Higher-order Frobenius coefficient fitting
      - a_F0 + a_F1*dt, b_F0 + b_F1*dt least-squares fitting
      - Higher-order composite model d_F_model_ho(dt)
      - Non-parametric law-of-cosines verification
      - Frobenius Richardson extrapolation
      - Comparison: simple vs higher-order model accuracy
    * Part 6g: unchanged (Frobenius angle estimation)
    * Part 6h IMPROVED: Higher-order model-based Rate(d_F) prediction
    * Part 6i: unchanged (T/d_F ratio analysis)
    * Part 6j EXPANDED: CT pairs included in Frobenius convergence rates
    * Parts 6a-6e retained from iteration 24

  Key issues addressed from iteration 26:
    1. Frobenius coefficient fitting used simple averaging:
       -> Higher-order fitting reduces composite model max error 1.13% -> ~0.23%.
       -> Rate(d_F) prediction max |Δ| reduced from 0.017 to ~0.006.
    2. No non-parametric verification of law of cosines:
       -> Added explicit verification showing machine-precision (~1e-16) accuracy.
    3. Missing Frobenius Richardson extrapolation:
       -> Added for consistency with trace-distance analysis (Part 6e).
    4. Missing Frobenius distance pairs (CT-ex, SCPT-CT):
       -> Added for complete Frobenius convergence rate analysis.
...
def test_exact_liouvillian_comparison(params: GKSLPhysicalParameters) -> dict:
    ...
    MODIFIED from iteration 26 (iteration 27):
    - Frobenius distance pairs EXPANDED: frob_CT_vs_exact, frob_SCPT_vs_CT added
    - Part 6f IMPROVED: Higher-order Frobenius coefficient fitting + non-parametric verification
    - Part 6h IMPROVED: Higher-order model-based Rate(d_F) prediction
    - Part 6j EXPANDED: CT pairs in Frobenius convergence rates
    - Parts 6a-6e, 6g, 6i retained from iteration 26
    """
    print("\n" + "=" * 70)
    print("Test E: Exact Liouvillian Comparison (ST vs Exact vs CT vs CPT vs SCPT)")
    print("       [iter 27: Higher-order Frobenius fitting + expanded Frobenius pairs]")
    ...
    # Part 6f: Frobenius-based composite model (IMPROVED in iteration 27)
    ...
    print(f"\n  Part 6f: Frobenius-based composite model (IMPROVED in iteration 27)")
    print(f"  Iteration 27 adds: higher-order fitting, non-parametric verification, Richardson extrapolation")
    ...
    if len(dts) >= 2:
        ...
        # --- Higher-order Frobenius coefficient fitting (NEW in iteration 27) ---
        a_F0_ho = a_F
        a_F1_ho = 0.0
        b_F0_ho = b_F
        b_F1_ho = 0.0
        ...
        if len(dts) >= 3:
            ...
            # Stinespring: d_F(ST,SCPT)/dt = a_F0 + a_F1*dt
            ...
            # Strang: d_F(SCPT,ex)/dt^2 = b_F0 + b_F1*dt
        ...
        print(f"\n  --- Higher-order model (NEW in iteration 27) ---")
        print(f"  Stinespring: d_F(ST,SCPT) = ({a_F0_ho:.6e} + {a_F1_ho:.6e}·dt) · dt")
        print(f"    a_F0 (asymptotic) = {a_F0_ho:.6e}, a_F1 (correction) = {a_F1_ho:.6e}")
        if frob_stine_ho_errors:
            print(f"    Relative errors: {', '.join(f'{e:.6f}' for e in frob_stine_ho_errors)}")
        print(f"  Strang: d_F(SCPT,ex) = ({b_F0_ho:.6e} + {b_F1_ho:.6e}·dt) · dt²")
        print(f"    b_F0 (asymptotic) = {b_F0_ho:.6e}, b_F1 (correction) = {b_F1_ho:.6e}")
        if frob_strang_ho_errors:
            print(f"    Relative errors: {', '.join(f'{e:.6f}' for e in frob_strang_ho_errors)}")
        print(f"  r_F (higher-order) = b_F0/a_F0 = {r_F_ho:.4f}")
        print(f"\n  Comparison: simple vs asymptotic coefficients:")
        print(f"    Stinespring: mean={a_F:.6e}, asymptotic a_F0={a_F0_ho:.6e}, diff={abs(a_F-a_F0_ho)/a_F*100:.4f}%")
        print(f"    Strang: mean={b_F:.6e}, asymptotic b_F0={b_F0_ho:.6e}, diff={abs(b_F-b_F0_ho)/b_F*100:.4f}%")
        ...
        # --- Model comparison: simple vs higher-order ---
        ...
        print(f"\n  --- Model comparison: simple vs higher-order vs trace-distance ---")
        print(f"  {'dt':>5} | {'d_F_simple':>11} | {'err_s':>7} | {'d_F_HO':>11} | {'err_HO':>7} | "
              f"{'d_F_obs':>11} | {'T_model':>11} | {'err_T':>7}")
        print("  " + "-" * 100)
        ...
        for k_idx, cd_entry in enumerate(convergence_data):
            ...
            # Simple Frobenius model (iteration 26)
            ...
            print(
                f"  {dt_val:5.2f} | {dF_model_dt:11.4e} | {rel_err_F:6.4f} | "
                f"{dF_ho_model_dt:11.4e} | {rel_err_F_ho:6.4f} | "
                f"{dF_obs_dt:11.4e} | {tT_model_dt:11.4e} | {rel_err_T:6.4f}"
            )

        print(f"\n  Simple Frobenius model max error: {max(dF_model_errors):.4f} ({max(dF_model_errors)*100:.2f}%)")
        print(f"  Higher-order Frobenius model max error: {max(dF_ho_model_errors):.4f} ({max(dF_ho_model_errors)*100:.2f}%)")
        print(f"  Trace-dist model max error: {max(tT_model_errors):.4f} ({max(tT_model_errors)*100:.1f}%)")
        if max(dF_ho_model_errors) > 0:
            print(f"  → Higher-order model is {max(dF_model_errors)/max(dF_ho_model_errors):.1f}x more accurate than simple")
        print(f"  → Frobenius model is {max(tT_model_errors)/max(dF_model_errors):.1f}x more accurate than trace-distance")
        ...
        composite_fit_result = {
            ...
            # Higher-order Frobenius model (NEW in iteration 27)
            ...
            # Non-parametric verification (NEW in iteration 27)
    ...
    # Part 6h: Frobenius-based Rate prediction (IMPROVED in iteration 27)
    print(f"\n  Part 6h: Frobenius-based Rate(d_F) prediction (IMPROVED in iteration 27)")
    print(f"  Rate(d_F) uses Frobenius model where law of cosines is exact.")
    print(f"  Both simple (iter 26) and higher-order (iter 27) model predictions shown.")
    ...
    if composite_fit_result and len(convergence_data) >= 2:
        ...
        print(f"  Higher-order: a_F0={a_F0_mod:.6e}, a_F1={a_F1_mod:.6e}")
        print(f"                b_F0={b_F0_mod:.6e}, b_F1={b_F1_mod:.6e}")
        print(f"\n  {'dt range':>15} | {'Rate_s(d_F)':>11} | {'Rate_HO(d_F)':>12} | {'Rate_obs(d_F)':>13} | "
              f"{'Δ_simple':>8} | {'Δ_HO':>8} | {'Rate_obs(T)':>11}")
        print("  " + "-" * 110)
        ...
        for k in range(1, len(convergence_data)):
            ...
            # Higher-order model Rate(d_F) (NEW in iteration 27)
            ...
            print(
                f"  {dt_prev:.1f} → {dt_curr:.1f}      | {rate_model_simple:11.4f} | {rate_model_ho:12.4f} | "
                f"{rate_obs_dF:13.4f} | {delta_rate_simple:+8.4f} | {delta_rate_ho:+8.4f} | "
                f"{rate_observed_T:11.4f}"
            )
        ...
        if delta_simples and delta_hos:
            print(f"\n  Rate prediction improvement: max|Δ_simple|={max(delta_simples):.4f}, max|Δ_HO|={max(delta_hos):.4f}")
            if max(delta_hos) > 0:
                print(f"  → Higher-order model is {max(delta_simples)/max(delta_hos):.1f}x more accurate")
...
def main() -> int:
    ...
    print("=" * 70)
    print("TTA-UC GKSL Iteration 27: Higher-order Frobenius Fitting")
    ...
    results: dict = {
        "timestamp": timestamp,
        "iteration": 27,
        "purpose": "Higher-order Frobenius coefficient fitting. "
                   "Iteration 26 simple-averaging model had 1.13% max error and Rate(d_F) "
                   "prediction max|Δ|=0.017. Root cause: simple averaging ignores higher-order "
                   "corrections in d_F(ST,SCPT)/dt and d_F(SCPT,ex)/dt². This iteration adds: "
                   "(1) Higher-order Frobenius fitting (a_F0+a_F1*dt, b_F0+b_F1*dt), "
                   "(2) Non-parametric law-of-cosines verification, "
                   "(3) Frobenius Richardson extrapolation, "
                   "(4) Expanded Frobenius distance pairs (CT-ex, SCPT-CT), "
                   "(5) Higher-order Rate(d_F) prediction.",
    ...
    # Write JSON report
    # ===================================================================
    json_path = output_dir / f"iteration27_ho_frobenius_{timestamp}.json"
    ...
    # ===================================================================
    md_lines = [
        "# TTA-UC GKSL Iteration 27: 高次 Frobenius フィッティング",
        "",
        f"- 実行時刻(UTC): {timestamp}",
        "- iteration 26 からの主要修正:",
        "  - Frobenius 距離ペア拡張: frob_CT_vs_exact, frob_SCPT_vs_CT を追加",
        "  - Part 6f 改善: 高次 Frobenius 係数フィッティング + 非パラメトリック検証 + Richardson 外挿",
        "  - Part 6h 改善: 高次モデルベース Rate(d_F) 予測",
        "  - Part 6j 拡張: CT ペアの Rate(d_F) 追加",
        "  - Parts 6a-6e, 6g, 6i: iteration 26 と同一",
        "",
    ]
    ...
    # Frobenius composite model (REBUILT in iteration 26)
    if te.get("composite_fit"):
        cf_data = te["composite_fit"]
        md_lines.extend([
            "### Frobenius ベース合成モデル（iteration 27 改善: 高次フィッティング）",
            "",
            "Frobenius ノルム（L₂）は内積空間ノルムであるため、余弦定理が厳密に成立する。",
            "",
            "#### 単純モデル（iteration 26 ベースライン）",
        ...
        # Higher-order model section (NEW in iteration 27)
        if cf_data.get("frobenius_a_F0_ho") is not None:
            md_lines.extend([
                "#### 高次モデル（iteration 27 新規）",
        ...
        # Richardson extrapolation (NEW in iteration 27)
        if cf_data.get("frobenius_a_F_richardson") is not None:
            md_lines.extend([
                "#### Frobenius Richardson 外挿（iteration 27 新規）",
                "",
        ...
        if cf_data.get("frobenius_nonparam_errors"):
            ...
            md_lines.extend([
                "#### 非パラメトリック余弦定理検証（iteration 27 新規）",
                "",
                f"余弦定理は機械精度で成立（最大誤差: {max(np_errs):.2e}）。",
                "パラメトリックモデルの誤差源は係数フィッティングと角度平均化のみ。",
                "",
            ])
        # Model comparison table
        md_lines.extend([
            "#### モデル比較: 単純 vs 高次 vs トレース距離",
            "",
            "| dt | d_F_simple | 誤差_s | d_F_HO | 誤差_HO | d_F_obs | T_model | 誤差_T |",
            "|----|-----------|-------|--------|--------|---------|---------|-------|",
        ])
        for k_idx, cd_entry in enumerate(te["convergence_data"]):
            dF_mod = cf_data["frobenius_model_dF_values"][k_idx]
            dF_obs = cd_entry["frob_ST_vs_exact"]
            err_F = cf_data["frobenius_model_relative_errors"][k_idx]
            dF_ho = cf_data.get("frobenius_ho_model_dF_values", cf_data["frobenius_model_dF_values"])[k_idx]
            err_ho = cf_data.get("frobenius_ho_model_relative_errors", cf_data["frobenius_model_relative_errors"])[k_idx]
            tT_mod = cf_data["trace_model_T_values"][k_idx]
            err_T = cf_data["trace_model_relative_errors"][k_idx]
            md_lines.append(
                f"| {cd_entry['dt']:.2f} | {dF_mod:.4e} | {err_F:.4f} | "
                f"{dF_ho:.4e} | {err_ho:.4f} | {dF_obs:.4e} | {tT_mod:.4e} | {err_T:.4f} |"
            )
        md_lines.append("")
    ...
    # Rate prediction (IMPROVED in iteration 27: simple + higher-order)
    if te.get("rate_prediction") and te["rate_prediction"].get("predictions"):
        rp = te["rate_prediction"]
        md_lines.extend([
            "### Frobenius ベース Rate(d_F) 予測と実測の比較（iteration 27 改善）",
            "",
            ...
            "",
            "| dt 範囲 | Rate_s(d_F) | Rate_HO(d_F) | Rate_obs(d_F) | Δ_simple | Δ_HO | Rate_obs(T) |",
            "|---------|-----------|------------|-------------|---------|------|-----------|",
        ])
        for pred in rp["predictions"]:
            delta_s_str = f"{pred.get('delta_rate_simple', pred.get('delta_rate_dF', float('nan'))):+.4f}"
            delta_ho_str = f"{pred.get('delta_rate_ho', float('nan')):+.4f}" if not np.isnan(pred.get("delta_rate_ho", float("nan"))) else "—"
            rate_s = pred.get("rate_model_simple", pred.get("rate_model_dF", float("nan")))
    ...
    md_path = output_dir / f"iteration27_ho_frobenius_{timestamp}.md"
    md_path.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
    ...
    if te.get("composite_fit"):
        ...
        if cf_data.get("frobenius_ho_model_relative_errors"):
            print(f"  Test E: Higher-order Frobenius model max_error={max(cf_data['frobenius_ho_model_relative_errors'])*100:.2f}% "
                  f"(improvement: {max(cf_data['frobenius_model_relative_errors'])/max(cf_data['frobenius_ho_model_relative_errors']):.1f}x)")
        if cf_data.get("frobenius_nonparam_errors"):
    ...
    if te.get("rate_prediction") and te["rate_prediction"].get("predictions"):
        rp = te["rate_prediction"]
        max_delta_simple = max(abs(p.get("delta_rate_simple", p.get("delta_rate_dF", float("nan"))))
                              for p in rp["predictions"]
                              if not np.isnan(p.get("delta_rate_simple", p.get("delta_rate_dF", float("nan")))))
        max_delta_ho = max(abs(p.get("delta_rate_ho", float("nan")))
                          for p in rp["predictions"]
                          if not np.isnan(p.get("delta_rate_ho", float("nan"))))
        print(f"  Test E: Rate(d_F) prediction max|Δ_simple|={max_delta_simple:.4f}, max|Δ_HO|={max_delta_ho:.4f}")
        if rp.get("dt_threshold_rate_dF_095") is not None:
