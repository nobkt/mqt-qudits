#!/usr/bin/env python3
"""TTA-UC GKSL iteration 28 verification: cos(θ_F) dt-dependent modeling.

Changes from iteration 27:
  - Test A-D: unchanged
  - Test E (extended analysis):
    * Part 6f IMPROVED: cos(θ_F)(dt) modeling + model variant comparison
      - cos(θ_F) = c0 + c1*dt² least-squares fitting
      - 4 composite model variants: simple, HO+cos_mean, HO+cos(dt²), HO+per-dt
      - Error decomposition: coefficient error vs cos(θ_F) approximation error
    * Part 6h IMPROVED: cos(dt²) model-based Rate(d_F) prediction added
    * All other parts retained from iteration 27

  Key issues addressed from iteration 27:
    1. HO composite model worse than simple model at small dt:
       - dt=0.1: HO 0.10% vs simple 0.006% (16x worse)
       - Root cause: HO coefficient fitting error + constant cos_mean = non-optimal
       -> cos(θ_F)(dt²) modeling reduces max error from 0.23% to ~0.15%.
    2. cos(θ_F) treated as constant (mean):
       - cos(θ_F) varies from -0.5730 (dt=2.0) to -0.5767 (dt=0.1)
       - dt² model fits with max error ~10⁻⁵ (vs cos variation 0.004)
       -> Added explicit cos(θ_F)(dt) = c0 + c1*dt² modeling.
    3. No error decomposition diagnostics:
       -> Added coefficient-error vs cos-error decomposition for each dt.
...
def test_exact_liouvillian_comparison(params: GKSLPhysicalParameters) -> dict:
    ...
    MODIFIED from iteration 27 (iteration 28):
    - Part 6f IMPROVED: cos(θ_F)(dt) modeling + model variant comparison + error decomposition
    - Part 6h IMPROVED: cos(dt²) model-based Rate(d_F) prediction added
    - All other parts retained from iteration 27
    """
    print("\n" + "=" * 70)
    print("Test E: Exact Liouvillian Comparison (ST vs Exact vs CT vs CPT vs SCPT)")
    print("       [iter 28: cos(θ_F)(dt) modeling + model variant comparison]")
    ...
    # Part 6f: Frobenius-based composite model (IMPROVED in iteration 28)
    ...
    print(f"\n  Part 6f: Frobenius-based composite model (IMPROVED in iteration 28)")
    print(f"  Iteration 28 adds: cos(θ_F)(dt) modeling, 4-model comparison, error decomposition")
    ...
    if len(dts) >= 2:
        ...
        print(f"  M2: HO+cos_mean (iter27) — HO coefficients, cos_mean")
        print(f"  M3: HO+cos(dt²) (iter28) — HO coefficients, cos(θ_F)(dt²)")
        print(f"  M4: HO+per-dt cos (reference) — HO coefficients, per-dt cos(θ_F)")
        print(f"  T:  Trace-dist model (iter25) — for comparison")
        print(f"\n  {'dt':>5} | {'M1 err%':>8} | {'M2 err%':>8} | {'M3 err%':>8} | {'M4 err%':>8} | {'T err%':>7} | {'d_F obs':>11}")
        print("  " + "-" * 80)
        ...
        for k_idx, cd_entry in enumerate(convergence_data):
            ...
            print(
                ...
                f"{rel_err_cos*100:7.4f}% | {rel_err_perdt*100:7.4f}% | "
                f"{rel_err_T*100:6.2f}% | {dF_obs_dt:11.4e}"
            )

        print(f"\n  Max errors across all dt:")
        print(f"    M1 Simple:      {max(dF_model_errors)*100:.4f}%")
        print(f"    M2 HO+cos_mean: {max(dF_ho_model_errors)*100:.4f}%")
        print(f"    M3 HO+cos(dt²): {max(dF_cos_dt2_model_errors)*100:.4f}%")
        print(f"    M4 HO+per-dt:   {max(dF_perdt_model_errors)*100:.4f}% (coefficient-fitting limit)")
        print(f"    T  Trace-dist:  {max(tT_model_errors)*100:.2f}%")
        if max(dF_cos_dt2_model_errors) > 0:
            print(f"  → M3 is {max(dF_ho_model_errors)/max(dF_cos_dt2_model_errors):.1f}x more accurate than M2 (cos improvement)")
            print(f"  → M3 is {max(dF_model_errors)/max(dF_cos_dt2_model_errors):.1f}x more accurate than M1 (overall)")
    ...
    # Part 6h: Frobenius-based Rate prediction (IMPROVED in iteration 28)
    print(f"\n  Part 6h: Frobenius-based Rate(d_F) prediction (IMPROVED in iteration 28)")
    print(f"  Rate(d_F) uses Frobenius model where law of cosines is exact.")
    print(f"  Models: simple (iter26), HO+cos_mean (iter27), HO+cos(dt²) (iter28).")
    ...
    if composite_fit_result and len(convergence_data) >= 2:
        ...
        print(f"\n  {'dt range':>15} | {'Rate_s':>7} | {'Rate_M2':>7} | {'Rate_M3':>7} | {'Rate_obs':>8} | "
              f"{'Δ_s':>8} | {'Δ_M2':>8} | {'Δ_M3':>8} | {'Rate_T':>7}")
        print("  " + "-" * 110)
        ...
        for k in range(1, len(convergence_data)):
            ...
            print(
                ...
                f"{rate_model_cos:7.4f} | {rate_obs_dF:8.4f} | {delta_rate_simple:+8.4f} | "
                f"{delta_rate_ho:+8.4f} | {delta_rate_cos:+8.4f} | {rate_observed_T:7.4f}"
            )
        ...
        if delta_simples and delta_hos and delta_cos_dt2s:
            print(f"\n  Rate prediction max|Δ|: simple={max(delta_simples):.4f}, M2={max(delta_hos):.4f}, M3={max(delta_cos_dt2s):.4f}")
...
def main() -> int:
    ...
    results: dict = {
        ...
        "purpose": "cos(θ_F)(dt) modeling. "
                   ...
                   "(2) 4-model variant comparison (simple, HO+cos_mean, HO+cos(dt²), HO+per-dt), "
    ...
    # Write JSON report
    # ===================================================================
    json_path = output_dir / f"iteration28_cos_dt2_{timestamp}.json"
    ...
    md_lines = [
        ...
        "  - Part 6f 改善: 4 モデルバリアント体系比較（単純, HO+cos_mean, HO+cos(dt²), HO+per-dt）",
        "  - Part 6f 改善: 誤差分解診断（係数誤差 vs cos 近似誤差）",
        "  - Part 6h 改善: cos(dt²) モデルベース Rate(d_F) 予測追加",
    ...
    if te.get("composite_fit"):
        ...
        md_lines.extend([
            "### Frobenius ベース合成モデル（iteration 28 改善: cos(θ_F)(dt) モデリング）",
            "",
        ...
        md_lines.extend([
            "#### 4 モデルバリアント比較（iteration 28 新規）",
            "",
            "| dt | M1 誤差% | M2 誤差% | M3 誤差% | M4 誤差% | T 誤差% | d_F obs |",
            "|----|---------|---------|---------|---------|--------|---------|",
        ])
        ...
        for k_idx, cd_entry in enumerate(te["convergence_data"]):
            ...
            md_lines.append(
                f"| {cd_entry['dt']:.2f} | {e1:.4f} | {e2:.4f} | {e3:.4f} | {e4:.4f} | {eT:.2f} | {dF_obs:.4e} |"
            )
        md_lines.extend([
            "",
            "- M1: 単純モデル（iter26）— 定数 a_F, b_F, cos_mean",
            "- M2: HO+cos_mean（iter27）— HO 係数, cos_mean",
            "- M3: HO+cos(dt²)（iter28）— HO 係数, cos(θ_F)(dt²)",
            "- M4: HO+per-dt cos（参照）— HO 係数, per-dt cos（係数フィッティング限界）",
            "- T: トレース距離モデル（iter25）",
            "",
        ])
    ...
    # Rate prediction (IMPROVED in iteration 28: simple + HO + cos(dt²))
    if te.get("rate_prediction") and te["rate_prediction"].get("predictions"):
        rp = te["rate_prediction"]
        md_lines.extend([
            "### Frobenius ベース Rate(d_F) 予測と実測の比較（iteration 28 改善）",
            "",
            ...
            "",
            "| dt 範囲 | Rate_M1 | Rate_M2 | Rate_M3 | Rate_obs | Δ_M1 | Δ_M2 | Δ_M3 | Rate_T |",
            "|---------|--------|--------|--------|---------|------|------|------|--------|",
        ])
        for pred in rp["predictions"]:
            ...
            md_lines.append(
                ...
                f"{rate_s:.4f} | {rate_ho:.4f} | {rate_cos:.4f} | "
    ...
    md_path = output_dir / f"iteration28_cos_dt2_{timestamp}.md"
    md_path.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
