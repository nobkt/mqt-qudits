#!/usr/bin/env python3
"""TTA-UC GKSL iteration 37 verification: shot-based palindromic consistency.

This iteration verifies the fixes from iteration 37:

  15. Shot-based (qudit) vs DM (qudit) consistency at n_steps=10 (2-molecule)
  16. Shot-based (qubit) vs DM (qubit) consistency at n_steps=10 (2-molecule)
  17. Noisy DM (qudit) zero-noise limit matches ideal DM
  18. Noisy DM (qubit) zero-noise limit matches ideal DM

All iteration-36 checks (1–14) are re-run for regression testing.
...
def main() -> None:
    ...
    results: dict = {
        "iteration": 37,
        "timestamp": timestamp,
        "description": "Palindromic consistency verification (checks 1-18)",
        "params": {
            "t_max": t_max,
    ...
    print("=" * 70)
    print("Iteration 37: Palindromic Consistency Verification")
    print("=" * 70)
    ...
    # ===================================================================
    # Part 10: NEW — Shot-based qudit vs DM qudit consistency (check 15)
    # ===================================================================
    print("\n--- Part 10: Shot-based qudit vs DM qudit consistency (check 15) ---")

    from qudit_gksl_shot_simulator import QuditGKSLShotSimulator

    # Use 2-molecule system for speed, high n_shots for statistical convergence
    ...
    # With 5000 shots and 9-dim system, statistical error should be small.
    # The key test: shot-based should converge to DM result within statistical noise.
    # For 2-mol system (dim=9), 5000 shots: expected error ~ O(1/sqrt(5000)) ~ 0.014
    shot_dm_ok = bool(td_shot_dm < 0.05)  # generous bound for statistical noise
    ...
    # ===================================================================
    # Part 11: NEW — Shot-based qubit vs DM qubit consistency (check 16)
    # ===================================================================
    ...
    # ===================================================================
    # Part 12: NEW — Noisy DM qudit zero-noise matches ideal (check 17)
    # ===================================================================
    ...
    # ===================================================================
    # Part 13: NEW — Noisy DM qubit zero-noise matches ideal (check 18)
    # ===================================================================
    ...
    # Save results
    # ===================================================================
    json_path = results_dir / f"iteration37_palindromic_verification_{timestamp}.json"
    with open(json_path, "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False, default=str)
    print(f"\nJSON results saved to: {json_path}")

    md_path = results_dir / f"iteration37_palindromic_verification_{timestamp}.md"
    md_lines = [
        f"# Iteration 37 検証結果: 回文順序一貫性検証\n",
        f"\n",
        f"- 実行日時: {timestamp}\n",
        f"- 検証対象: 全18チェック（既存14 + 新規4）\n",
        f"\n",
        f"## 収束解析結果（4分子系）\n",
        f"\n",
