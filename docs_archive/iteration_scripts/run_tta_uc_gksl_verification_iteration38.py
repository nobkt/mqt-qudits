#!/usr/bin/env python3
"""TTA-UC GKSL iteration 38 verification: metadata correctness.

This iteration verifies the fixes from iteration 38:

  19. Qudit DM ancilla count matches Lindblad operator count (4-molecule)
  20. Qudit DM ancilla count matches Lindblad operator count (2-molecule)
  21. Qudit DM gate count is dynamically computed (4-molecule)
  22. Qudit DM gate count is dynamically computed (2-molecule)
  23. Qudit Shot gate count is dynamically computed (4-molecule)
  24. Qudit Shot gate count is dynamically computed (2-molecule)
  25. Qubit Shot includes gate count estimation
  26. Qudit-Qubit Shot identical results with same seed (2-molecule)

All iteration-37 checks (1–18) are re-run for regression testing.
...
def main() -> None:
    ...
    results: dict = {
        "iteration": 38,
        "timestamp": timestamp,
        "description": "Metadata correctness verification (checks 1-26)",
        "params": {
            "t_max": t_max,
    ...
    print("=" * 70)
    print("Iteration 38: Metadata Correctness Verification")
    print("=" * 70)
    ...
    # Save results
    # ===================================================================
    json_path = results_dir / f"iteration38_metadata_verification_{timestamp}.json"
    with open(json_path, "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False, default=str)
    print(f"\nJSON results saved to: {json_path}")

    md_path = results_dir / f"iteration38_metadata_verification_{timestamp}.md"
    md_lines = [
        f"# Iteration 38 検証結果: メタデータ正確性検証\n",
        f"\n",
        f"- 実行日時: {timestamp}\n",
