#!/usr/bin/env python3
"""TTA-UC GKSL iteration 39 verification: documentation update.

This iteration verifies the fixes from iteration 39:

  - Notebook Cell 25/39 iteration count updated from "34回" to "38回"

All iteration-38 checks (1–26) are re-run for regression testing.
...
def check_documentation_fixes(notebook_path: str) -> dict:
    ...
    has_38_cell25 = "38回のイテレーション検証" in cell25_text
    has_old_34_cell25 = "34回のイテレーション検証" in cell25_text
    results["cell25_has_38"] = has_38_cell25
    results["cell25_has_old_34"] = has_old_34_cell25
    results["cell25_ok"] = has_38_cell25 and not has_old_34_cell25

    cell39_text = "".join(nb["cells"][39]["source"])
    has_38_cell39 = "38回の検証イテレーション" in cell39_text
    has_old_34_cell39 = "34回の検証イテレーション" in cell39_text
    results["cell39_has_38"] = has_38_cell39
    results["cell39_has_old_34"] = has_old_34_cell39
    results["cell39_ok"] = has_38_cell39 and not has_old_34_cell39
...
def main() -> None:
    ...
    results: dict = {
        "iteration": 39,
        "timestamp": timestamp,
        "description": "Documentation update verification (checks 1-26)",
        "params": {
            "t_max": t_max,
    ...
    print("=" * 70)
    print("Iteration 39: Documentation Update Verification")
    print("=" * 70)
    ...
    print(f"  Check 7 - Cell 25 has '38回': {doc_fixes['cell25_ok']}")
    print(f"  Check 8 - Cell 39 has '38回': {doc_fixes['cell39_ok']}")
    ...
    # Save results
    # ===================================================================
    json_path = results_dir / f"iteration39_documentation_verification_{timestamp}.json"
    with open(json_path, "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False, default=str)
    print(f"\nJSON results saved to: {json_path}")

    md_path = results_dir / f"iteration39_documentation_verification_{timestamp}.md"
    md_lines = [
        f"# Iteration 39 検証結果: 文書更新検証\n",
        f"\n",
        f"- 実行日時: {timestamp}\n",
    ...
    md_lines.extend([
        ...
        f"| 7 | Cell 25 「38回」表記 | {'✓' if doc_fixes['cell25_ok'] else '✗'} |\n",
        f"| 8 | Cell 39 「38回」表記 | {'✓' if doc_fixes['cell39_ok'] else '✗'} |\n",
