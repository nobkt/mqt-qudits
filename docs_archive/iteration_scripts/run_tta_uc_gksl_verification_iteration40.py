#!/usr/bin/env python3
"""TTA-UC GKSL iteration 40 verification: iteration count update + dead code removal.

This iteration verifies the fixes from iteration 40:

  - Notebook Cell 25/39 iteration count updated from "38回" to "39回"
  - Dead code `build_trotter_step_classical` removed from stinespring_utils.py

All iteration-39 checks (1–26) are re-run for regression testing.
...
def check_documentation_fixes(notebook_path: str) -> dict:
    ...
    has_39_cell25 = "39回のイテレーション検証" in cell25_text
    has_old_38_cell25 = "38回のイテレーション検証" in cell25_text
    results["cell25_has_39"] = has_39_cell25
    results["cell25_has_old_38"] = has_old_38_cell25
    results["cell25_ok"] = has_39_cell25 and not has_old_38_cell25

    cell39_text = "".join(nb["cells"][39]["source"])
    has_39_cell39 = "39回の検証イテレーション" in cell39_text
    has_old_38_cell39 = "38回の検証イテレーション" in cell39_text
    results["cell39_has_39"] = has_39_cell39
    results["cell39_has_old_38"] = has_old_38_cell39
    results["cell39_ok"] = has_39_cell39 and not has_old_38_cell39
...
def main() -> None:
    ...
    results: dict = {
        "iteration": 40,
        "timestamp": timestamp,
        "description": "Iteration count update + dead code removal verification (checks 1-26)",
        "params": {
            "t_max": t_max,
    ...
    print("=" * 70)
    print("Iteration 40: Iteration Count Update + Dead Code Removal Verification")
    print("=" * 70)
    ...
    print(f"  Check 7 - Cell 25 has '39回': {doc_fixes['cell25_ok']}")
    print(f"  Check 8 - Cell 39 has '39回': {doc_fixes['cell39_ok']}")
    ...
    # Save results
    # ===================================================================
    json_path = results_dir / f"iteration40_verification_{timestamp}.json"
    with open(json_path, "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False, default=str)
    print(f"\nJSON results saved to: {json_path}")

    md_path = results_dir / f"iteration40_verification_{timestamp}.md"
    md_lines = [
        f"# Iteration 40 検証結果: イテレーション回数更新 + デッドコード削除\n",
        f"\n",
        f"- 実行日時: {timestamp}\n",
        f"- 検証対象: 全26チェック（既存18 + 新規8）\n",
        f"\n",
        f"## 収束解析結果（4分子系）\n",
        f"\n",
    ...
    md_lines.extend([
        ...
        f"| 7 | Cell 25 「39回」表記 | {'✓' if doc_fixes['cell25_ok'] else '✗'} |\n",
        f"| 8 | Cell 39 「39回」表記 | {'✓' if doc_fixes['cell39_ok'] else '✗'} |\n",
