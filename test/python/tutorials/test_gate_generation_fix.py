...
def test_h_transfer_generates_customtwo():
    ...
    assert gate_info['structure_type'] == 'sparse_2x2', \
        f"Expected sparse_2x2, got {gate_info['structure_type']}"
    
    # Should generate CustomTwo gate (not R gates with invalid indices)
    assert len(gate_info['gates']) == 1, \
        f"Expected 1 CustomTwo gate, got {len(gate_info['gates'])} gates"
    
    gate = gate_info['gates'][0]
    assert gate['type'] == 'CustomTwo', \
        f"Expected CustomTwo gate, got {gate['type']}"
    
    # The unitary should be preserved
    assert 'unitary' in gate['params'], "CustomTwo should have unitary parameter"
    assert np.allclose(gate['params']['unitary'], U_transfer), \
        "Unitary matrix should be preserved"
    
    print(f"  ✓ Structure type: {gate_info['structure_type']}")
    print(f"  ✓ Gate type: {gate['type']}")
    print(f"  ✓ Fidelity: {gate_info['fidelity']:.10f}")
    print(f"  ✓ No R gates with invalid indices (AssertionError avoided!)")
    print()
...
def test_h_tta_generates_customtwo():
    ...
    assert gate_info['structure_type'] == 'sparse_3x3', \
        f"Expected sparse_3x3, got {gate_info['structure_type']}"
    ...
    for gate in gate_info['gates']:
        if gate['type'] in ['R', 'Rz', 'Rh']:
            if 'level1' in gate['params']:
    ...
    print(f"  ✓ Fidelity: {gate_info['fidelity']:.10f}")
    print(f"  ✓ All R gates have valid indices (< 3)")
    print()
...
def test_single_qudit_subspace():
    ...
    assert customtwo_count == 0, \
        f"Single-qudit subspace should not use CustomTwo, but found {customtwo_count}"
    ...
    for gate in gate_info['gates']:
        if gate['type'] in ['R', 'Rz', 'Rh'] and 'level1' in gate['params']:
            ...
            assert level1 != level2, f"level1 and level2 should be different"
            print(f"  ✓ R gate with local indices: [{level1}, {level2}]")
    
    print(f"  ✓ Structure type: {gate_info['structure_type']}")
    print(f"  ✓ No CustomTwo gates (single-qudit optimization)")
    print(f"  ✓ All indices are valid local qudit levels")
    print()
