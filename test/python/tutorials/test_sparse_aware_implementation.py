...
def test_gate_count_reduction():
    ...
    # 2×2部分空間として検出されるべき
    assert gate_info["structure_type"] == "sparse_2x2", f"Expected sparse_2x2 but got {gate_info['structure_type']}"

    # ゲート数は小さいはず（1-5ゲート程度）
    assert gate_info["gate_count"] <= 5, f"Gate count too high: {gate_info['gate_count']}"
...
def test_decompose_custom_two_gates_method_exists():
    ...
    # メソッドが存在することを確認
    assert hasattr(time_evol, "decompose_custom_two_gates"), "decompose_custom_two_gatesメソッドが存在しません"

    # メソッドが呼び出し可能であることを確認
    assert callable(time_evol.decompose_custom_two_gates), "decompose_custom_two_gatesが呼び出し可能ではありません"

    print("✓ decompose_custom_two_gatesメソッド存在確認テスト合格")
