    ...
    def convert(self, params: Dict, active_indices: List[int]) -> MQTGateSequence:
        ...
        if abs(phase1_l1) > self.tolerance:
            gates.append(MQTGate(
                gate_type='VirtRz',
                parameters={'level': level1, 'phase': phase1_l1},
                cost=0
            ))
        ...
        if abs(phase1_l2) > self.tolerance:
            gates.append(MQTGate(
                gate_type='VirtRz',
                parameters={'level': level2, 'phase': phase1_l2},
                cost=0
            ))
        ...
        if abs(phase2_l1) > self.tolerance:
            gates.append(MQTGate(
                gate_type='VirtRz',
                parameters={'level': level1, 'phase': phase2_l1},
                cost=0
            ))
        ...
        if abs(phase2_l2) > self.tolerance:
            gates.append(MQTGate(
                gate_type='VirtRz',
                parameters={'level': level2, 'phase': phase2_l2},
                cost=0
            ))
        
        return MQTGateSequence(gates=gates, fidelity=1.0)
    
    def verify_conversion(self, original_U: np.ndarray,
                         gates: MQTGateSequence,
...
def test_2x2_gate_conversion():
    ...
    try:
        ...
        print("エラー: improved_unitary_decomposition.py が見つかりません")
        return False
    ...
    for i in range(num_tests):
        ...
        if i < 3:  # 最初の3つを詳細表示
            ...
            print(f"    λ = {result.lam:.6f}")
            print(f"    α = {result.global_phase:.6f}")
            print(f"  ゲート数: {gates.get_gate_count()} (物理: {gates.get_physical_gate_count()})")
            print(f"  忠実度: {fidelity:.10f}")
    
    # 統計
    print(f"\n統計（{num_tests}テスト）:")
    print(f"  最小忠実度: {min(fidelities):.10f}")
    print(f"  平均忠実度: {np.mean(fidelities):.10f}")
    print(f"  最大ゲート数: {max(gate_counts)}")
    print(f"  平均ゲート数: {np.mean(gate_counts):.2f}")
    
    # 合否判定
    pass_count = sum(1 for f in fidelities if f > 0.9999)
    print(f"\n合格率: {pass_count}/{num_tests} ({100*pass_count/num_tests:.1f}%)")
    
    if pass_count == num_tests and max(gate_counts) <= 3:
        print("✓ 2×2ゲート変換テスト合格")
        return True
    else:
        print("✗ 2×2ゲート変換テスト不合格")
        return False
...
def test_3x3_gate_conversion():
    ...
    for i in range(num_tests):
        ...
        if i < 3:  # 最初の3つを詳細表示
            print(f"\nテスト {i+1}:")
            print(f"  Givens回転数: {len(rotations)}")
            for j, (l1, l2, theta, phi) in enumerate(rotations):
                print(f"    G({l1},{l2}): θ={theta:.6f}, φ={phi:.6f}")
            print(f"  対角位相: [{', '.join(f'{p:.6f}' for p in phases)}]")
            print(f"  ゲート数: {gates.get_gate_count()} (物理: {gates.get_physical_gate_count()})")
            print(f"  忠実度: {fidelity:.10f}")
    ...
    if pass_count == num_tests and max(gate_counts) <= 12:
        print("✓ 3×3ゲート変換テスト合格")
        return True
    else:
        print("✗ 3×3ゲート変換テスト不合格")
        return False


def test_h_transfer_conversion():
    """H_transferユニタリでのゲート変換テスト"""
    print("\n" + "="*70)
    print("H_transferゲート変換テスト")
    print("="*70)
    ...
    print(f"\n変換後の忠実度: {fidelity:.10f}")
    
    if fidelity > 0.9999 and gates.get_gate_count() <= 3:
        print("✓ H_transferゲート変換テスト合格")
        return True
    else:
        print("✗ H_transferゲート変換テスト不合格")
        return False


def test_h_tta_conversion():
    """H_TTAユニタリでのゲート変換テスト"""
    print("\n" + "="*70)
    print("H_TTAゲート変換テスト")
    print("="*70)
    ...
    print(f"\nGivens分解:")
    print(f"  QR忠実度: {qr_result.fidelity:.10f}")
    print(f"  Givens回転数: {len(rotations)}")
    for l1, l2, theta, phi in rotations:
        print(f"    G({l1},{l2}): θ={theta:.6f}, φ={phi:.6f}")
    print(f"  対角位相: [{', '.join(f'{p:.6f}' for p in phases_diag)}]")
    ...
    if fidelity > 0.9999 and gates.get_gate_count() <= 12:
        print("✓ H_TTAゲート変換テスト合格")
        return True
    else:
        print("✗ H_TTAゲート変換テスト不合格")
        return False
...
def run_all_tests():
    """すべてのテストを実行"""
    print("\n" + "="*70)
    print("MQT-Quditsゲート変換器 - 包括的テスト")
    print("PR#39 Phase 2実装")
    print("="*70)
    ...
    if all_passed:
        ...
        print("\nPR#39 Phase 2完了:")
        print("  - 2×2ゲート変換: 忠実度 1.0, ≤3ゲート")
        print("  - 3×3ゲート変換: 忠実度 1.0, ≤12ゲート")
        print("  - 実問題検証: H_transfer/H_TTA両方で合格")
        print("  - 次のステップ: Phase 3（MQT-Qudits統合）")
    else:
        print("✗ 一部のテストが失敗しました")
    print("="*70)
