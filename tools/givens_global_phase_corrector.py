...
class GivensGlobalPhaseCorrector:
    ...
    def check_phase_correction_needed(self, G_target: np.ndarray, U_zyz: np.ndarray) -> tuple[bool, float]:
    ...
    def correct_zyz_global_phase(self, theta_givens: float, phi_givens: float, zyz_params: dict) -> dict:
        ...
        G_target = np.array([[c, s], [-np.conj(s), np.conj(c)]])
        ...
        needs_correction, phase_correction = self.check_phase_correction_needed(G_target, U_zyz)
    ...
    def verify_correction(
        self, theta_givens: float, phi_givens: float, zyz_params_original: dict, zyz_params_corrected: dict
    ) -> dict:
...
def test_corrector() -> bool:
    ...
    all_passed = True
    for theta, phi in failing_cases:
