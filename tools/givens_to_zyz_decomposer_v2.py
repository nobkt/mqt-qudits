...
class GivensToZYZDecomposerV2:
    ...
    def __init__(self, tolerance: float = 1e-10):
        ...
        try:
            ...
            print(f"警告: givens_global_phase_corrector.pyをインポートできません: {e}")
            print(f"  グローバル位相補正は利用できません")
    ...
    def decompose(self, theta: float, phi: float) -> Dict:
        ...
        if self.corrector_available:
            zyz_corrected = self.phase_corrector.correct_zyz_global_phase(
                theta, phi, zyz_params
            )
            # 補正が適用されたかチェック
...
def test_single_givens():
    """単一のGivens回転でテスト"""
    print("="*70)
    print("Givens → ZYZ分解のテスト (v2 - グローバル位相補正付き)")
    print("="*70)
    ...
    for idx, (theta, phi) in enumerate(test_cases, 1):
        ...
        corrected_mark = " (補正済み)" if zyz['phase_corrected'] else ""
        print(f"  ZYZ分解: θ={zyz['theta']:.6f}, φ={zyz['phi']:.6f}, λ={zyz['lambda']:.6f}{corrected_mark}")
    ...
    if all_passed:
        print("✓✓✓ すべてのテストに合格")
        print(f"グローバル位相補正: {correction_count}/{len(test_cases)} ケース")
    else:
        print("⚠⚠⚠ 一部のテストが不合格")
    print("="*70)
...
def test_random_givens(num_tests: int = 100):
    ...
    print(f"  グローバル位相補正: {correction_count}/{num_tests} ケース ({100*correction_count/num_tests:.1f}%)")
...
def main():
    """メイン関数"""
    print("="*70)
    print("Givens → ZYZ → MQT-Quditsゲート変換器 (v2)")
    print("="*70)
    print("\nPR#42: グローバル位相補正を統合し、100% pass rateを達成")
    print("\n手順:")
    print("1. Givens(θ, φ) → 2×2ユニタリ")
    print("2. 2×2ユニタリ → ZYZ分解（improved_unitary_decomposition.py）")
    print("3. グローバル位相補正（givens_global_phase_corrector.py）")
    print("4. ZYZ → MQT-Quditsゲート（VirtRz + R）")
    print("\n期待: 忠実度 1.0 を100%のケースで保証")
