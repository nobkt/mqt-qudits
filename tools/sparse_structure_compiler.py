...
class ThreeLevelRotationDecomposer:
    ...
    def _construct_givens_matrix(self, d: int, level1: int, level2: int, 
        ...
        if structure.structure_type == 'identity':
            ...
            if structure.active_dimension == 2:
                ...
                result['gate_sequence'] = [
                    (structure.active_subspace, rotations)
                ]
...
def test_h_transfer_structure():
    """H_transferの疎構造解析テスト"""
    print("="*70)
    print("H_transfer 疎構造解析テスト")
    print("="*70)
    ...
    print(f"\n1. 行列の次元: {U.shape[0]}×{U.shape[0]}")
    print(f"2. 回転角 θ = {theta:.4f}")
    ...
    print(f"\n3. 構造解析結果:")
    print(f"   - 構造タイプ: {structure.structure_type}")
    print(f"   - 全体の次元: {structure.dimension}")
    print(f"   - 作用する部分空間の次元: {structure.active_dimension}")
    print(f"   - 作用する基底インデックス: {structure.active_subspace}")
    print(f"   - 恒等変換の基底数: {len(structure.identity_indices)}")
    
    # 最適化見積もり
    current, optimized, reduction = estimate_gate_reduction(U)
    
    print(f"\n4. ゲート数見積もり:")
    print(f"   - 現在の分解: 約{current}ゲート")
    print(f"   - 最適化後: 約{optimized}ゲート")
    print(f"   - 削減率: {reduction*100:.1f}%")
    print(f"   - 削減倍率: {current/optimized:.1f}倍")
...
def test_h_tta_structure():
    ...
    print(f"\n1. 行列の次元: {U.shape[0]}×{U.shape[0]}")
    print(f"2. TTA相互作用定数 J = {J} eV")
    print(f"3. 時間ステップ dt = {dt} fs")
...
def test_mathematical_rigor():
    ...
    fidelity = abs(inner_product) / 2  # 2次元の場合
    error = np.max(np.abs(U_2x2 - U_reconstructed * (inner_product / abs(inner_product))))
    
    print(f"   - 忠実度: {fidelity:.6f}")
    print(f"   - 最大誤差: {error:.2e}")
    print(f"   - 厳密性: {'✓ 合格' if error < 1e-8 else '✗ 不合格'}")
    ...
    print(f"   - 分解された回転数: {len(rotations)}")
    print(f"   - 回転リスト:")
    for i, (l1, l2, theta, phi) in enumerate(rotations):
        print(f"     [{i}] 準位{l1}-{l2}: θ={theta:.4f}, φ={phi:.4f}")
    ...
    # QR分解後の上三角化を検証
    error_3x3 = np.max(np.abs(np.triu(U_3x3) - np.triu(U_reconstructed_3x3)))
    print(f"   - 上三角化誤差: {error_3x3:.2e}")
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("疎構造認識型Quditコンパイラ - テストスイート")
    print("="*70)
    print()
    ...
    print("="*70)
    print()
    print("このツールは、疎構造を持つCustomTwoゲートの効率的な分解を提供します。")
    print("数学的厳密性を保ちながら、ゲート数を大幅に削減することができます。")
    print()
    print("次のステップ:")
    print("1. MQT-Quditsフレームワークへの統合")
    print("2. 実際の量子回路での検証")
