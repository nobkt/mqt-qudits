...
class RigorousTwoQubitDecomposer:
    ...
    def decompose_zyz(U: np.ndarray, tolerance: float = 1e-10) -> UnitaryDecomposition2x2:
        ...
        # 再構築して忠実度を計算
        U_reconstructed = RigorousTwoQubitDecomposer._construct_zyz(theta, phi, lam, global_phase)
        fidelity = RigorousTwoQubitDecomposer._compute_fidelity(U, U_reconstructed)

        return UnitaryDecomposition2x2(
            theta=theta, phi=phi, lam=lam, global_phase=global_phase, fidelity=fidelity, method="ZYZ"
        )
    ...
    def decompose_zxz(U: np.ndarray, tolerance: float = 1e-10) -> UnitaryDecomposition2x2:
        ...
        # ZXZ分解
        u00, _u01 = U_normalized[0, 0], U_normalized[0, 1]
        _u10, u11 = U_normalized[1, 0], U_normalized[1, 1]
        ...
        # 再構築して忠実度を計算
        U_reconstructed = RigorousTwoQubitDecomposer._construct_zxz(theta, phi, lam, global_phase)
        fidelity = RigorousTwoQubitDecomposer._compute_fidelity(U, U_reconstructed)

        return UnitaryDecomposition2x2(
            theta=theta, phi=phi, lam=lam, global_phase=global_phase, fidelity=fidelity, method="ZXZ"
        )
    ...
    @staticmethod
    def _construct_zxz(theta: float, phi: float, lam: float, global_phase: float = 0.0) -> np.ndarray:
        """ZXZ分解からユニタリ行列を構築.
        ...
        # Rx(θ)
        Rx_theta = np.array(
            [[np.cos(theta / 2), -1j * np.sin(theta / 2)], [-1j * np.sin(theta / 2), np.cos(theta / 2)]], dtype=complex
        )
...
class RigorousThreeQuditDecomposer:
    ...
    def decompose_givens(U: np.ndarray, tolerance: float = 1e-10) -> UnitaryDecomposition3x3:
        ...
        if abs(U_work[1, 0]) > tolerance:
            theta, phi = RigorousThreeQuditDecomposer._compute_givens_params(U_work[0, 0], U_work[1, 0])
            rotations.append((0, 1, theta, phi))

            # 回転行列を適用
            G = RigorousThreeQuditDecomposer._construct_givens_matrix(3, 0, 1, theta, phi)
            U_work = G.conj().T @ U_work

        # ステップ2: U[2,0]をゼロにする（準位0と2の回転）
        if abs(U_work[2, 0]) > tolerance:
            theta, phi = RigorousThreeQuditDecomposer._compute_givens_params(U_work[0, 0], U_work[2, 0])
            rotations.append((0, 2, theta, phi))

            G = RigorousThreeQuditDecomposer._construct_givens_matrix(3, 0, 2, theta, phi)
            U_work = G.conj().T @ U_work

        # ステップ3: U[2,1]をゼロにする（準位1と2の回転）
        if abs(U_work[2, 1]) > tolerance:
            theta, phi = RigorousThreeQuditDecomposer._compute_givens_params(U_work[1, 1], U_work[2, 1])
            rotations.append((1, 2, theta, phi))

            G = RigorousThreeQuditDecomposer._construct_givens_matrix(3, 1, 2, theta, phi)
            U_work = G.conj().T @ U_work
        ...
        # 再構築して忠実度を計算
        U_reconstructed = RigorousThreeQuditDecomposer._reconstruct_from_givens(rotations, diagonal_phases)
        fidelity = RigorousThreeQuditDecomposer._compute_fidelity(U, U_reconstructed)

        return UnitaryDecomposition3x3(
            rotations=rotations, diagonal_phases=diagonal_phases, fidelity=fidelity, method="Givens"
        )

    @staticmethod
    def _compute_givens_params(a: complex, b: complex, tolerance: float = 1e-15) -> tuple[float, float]:
        """Givens回転のパラメータを計算.
    ...
    @staticmethod
    def _reconstruct_from_givens(
        rotations: list[tuple[int, int, float, float]], diagonal_phases: np.ndarray
    ) -> np.ndarray:
        ...
        for level1, level2, theta, phi in rotations:
            G = RigorousThreeQuditDecomposer._construct_givens_matrix(3, level1, level2, theta, phi)
            U = G @ U
...
def test_rigorous_2x2_decomposition() -> None:
    """厳密な2×2分解のテスト."""
    # テストケース1: ランダムなユニタリ
    ...
    RigorousTwoQubitDecomposer.decompose_zyz(U_random)

    RigorousTwoQubitDecomposer.decompose_zxz(U_random)

    # テストケース2: H_transfer型の回転
    theta = 0.1
    U_transfer = np.array([[np.cos(theta), -1j * np.sin(theta)], [-1j * np.sin(theta), np.cos(theta)]], dtype=complex)

    RigorousTwoQubitDecomposer.decompose_zyz(U_transfer)


def test_rigorous_3x3_decomposition() -> None:
    """厳密な3×3分解のテスト."""
    # テストケース1: ランダムなユニタリ
    from scipy.stats import unitary_group
    ...
    for _i, (_l1, _l2, _theta, _phi) in enumerate(decomp.rotations):
        pass
    ...
    RigorousThreeQuditDecomposer.decompose_givens(U_tta)
