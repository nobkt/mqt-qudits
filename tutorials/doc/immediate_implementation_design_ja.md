...
class ImprovedTwoQubitDecomposer:
    ...
    @staticmethod
    def reconstruct_zyz(
        theta: float, phi: float, lam: float, alpha: float = 0.0
    ) -> np.ndarray:
        """
        ZYZ分解からユニタリを再構築
        ...
        # Rz(λ)
        Rz_lam = np.array(
            [[np.exp(1j * lam / 2), 0], [0, np.exp(-1j * lam / 2)]], dtype=complex
        )
    ...
    def verify_decomposition(
        U_original: np.ndarray, theta: float, phi: float, lam: float, alpha: float
    ) -> float:
        """
        分解結果を検証
...
def test_improved_2x2_decomposition():
    ...
    print(
        f"  - 合格率: {sum(1 for f in fidelities if f > 0.9999) / num_tests * 100:.1f}%"
    )
...
class ImprovedThreeQuditDecomposer:
    ...
    def decompose_accurate(U: np.ndarray) -> Dict:
        ...
        """
        # Step 1: ユニタリ性の検証
        assert ImprovedThreeQuditDecomposer._is_unitary(
            U
        ), "入力行列がユニタリではありません"
    ...
    @staticmethod
    def _construct_givens_accurate(
        d: int, level1: int, level2: int, theta: float, phi: float
    ) -> np.ndarray:
        """
        高精度Givens行列構築
        """
