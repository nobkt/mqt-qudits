...
def zyz_to_mqt_gates(
    theta: float, phi: float, lam: float, global_phase: float, level1: int, level2: int
) -> List[MQTGate]:
    """
    ...
    # R(θ, 0)
    if abs(theta) > 1e-10:
        gates.append(RGate(theta=theta, phi=0.0, level1=level1, level2=level2))
...
def test_2x2_gate_conversion():
    ...
    gates = zyz_to_mqt_gates(
        result.theta, result.phi, result.lam, result.global_phase, level1=0, level2=1
    )
...
def embed_gates_in_9x9(
    ...
    for gate in gates:
        if isinstance(gate, VirtRzGate):
            ...
            embedded_gates.append(VirtRzGate(phase=gate.phase, level=global_level))
...
def test_full_conversion_pipeline():
    ...
    # 部分空間抽出
    U_sub = analyzer.extract_subspace_unitary(U_transfer_9x9, structure.active_subspace)
...
class OptimizedGateBuilder:
    ...
    @staticmethod
    def add_virtrz(circuit: QuantumCircuit, qudit_index: int, level: int, phase: float):
        """VirtRzゲートを追加"""
...
class SparseStructureOptimizationPass(CompilerPass):
    ...
    def _optimize_gate(
        self, gate: CustomTwoGate, structure: SparseStructureInfo
    ) -> List[Gate]:
        """ゲートの最適化"""
        # 部分空間抽出
