...
class TwoLevelRotationDecomposer:
    ...
    def decompose_2x2_unitary(U: np.ndarray) -> Dict:
        ...
        return {"theta": theta, "phi": phi, "lambda": lam, "global_phase": global_phase}
```
...
class ThreeLevelRotationDecomposer:
    ...
    def _extract_givens_from_q(Q: np.ndarray) -> List[Tuple[int, int, float, float]]:
        ...
        if abs(b) > 1e-10:
            ...
            # Q†を更新
            G_01_dag = (
                ThreeLevelRotationDecomposer._construct_givens(
    ...
    @staticmethod
    def _construct_givens(
        d: int, i: int, j: int, theta: float, phi: float
    ) -> np.ndarray:
        """Givens行列を構築（検証用）"""
