...
```python
{
    "Q": np.ndarray,  # ユニタリ行列
    "R": np.ndarray,  # 上三角行列
    "rotations": [(level1, level2, theta, phi), ...],  # Givens回転のリスト
...
```python
def convert_2x2_to_mqt_gates(params: Dict, active_indices: List[int]) -> List[MQTGate]:
    """
    ...
    phase1 = alpha + phi
    if abs(phase1) > 1e-10:
        gates.append({"type": "VirtRz", "phase": phase1, "level": level1, "cost": 0})
    ...
    # VirtRz(λ, level2)
    if abs(lam) > 1e-10:
        gates.append({"type": "VirtRz", "phase": lam, "level": level2, "cost": 0})
...
```python
def verify_2x2_conversion(
    original_U: np.ndarray, gates: List[MQTGate], active_indices: List[int]
) -> float:
    """
    変換の正確性を検証
    ...
    for gate in gates:
        if gate["type"] == "VirtRz":
            ...
            if gate["level"] == active_indices[0]:
                U_reconstructed = (
                    np.diag([np.exp(1j * gate["phase"]), 1.0]) @ U_reconstructed
                )
            else:
                U_reconstructed = (
                    np.diag([1.0, np.exp(1j * gate["phase"])]) @ U_reconstructed
                )
...
```python
def convert_3x3_to_mqt_gates(params: Dict, active_indices: List[int]) -> List[MQTGate]:
    """
    ...
    for level1, level2, theta, phi in params["rotations"]:
        ...
        if abs(alpha) > 1e-10:
            gates.append(
                {"type": "VirtRz", "phase": alpha, "level": global_level1, "cost": 0}
            )
        ...
        if abs(beta) > 1e-10:
            gates.append(
                {"type": "VirtRz", "phase": beta, "level": global_level2, "cost": 0}
            )
    ...
    for local_level, phase in enumerate(params["diagonal_phases"]):
        ...
        if abs(phase) > 1e-10:
            gates.append(
                {"type": "VirtRz", "phase": phase, "level": global_level, "cost": 0}
            )
...
```python
def embed_gates_in_9x9(
    gates: List[MQTGate], active_indices: List[int], total_dimension: int = 9
) -> List[MQTGate]:
    """
    部分空間のゲートを完全な空間に埋め込み
