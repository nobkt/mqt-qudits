...
class SparseStructureAnalyzer:
    ...
    def analyze(self, U: np.ndarray, tolerance: float = 1e-10) -> dict:
        ...
        if structure["type"] == "sparse_subspace":
            # 2x2, 3x3などのブロック構造を検出
            structure["structure"] = self._analyze_subspace_structure(U, active_indices)
...
class SparseStructureDecomposer:
    ...
    def decompose(
        self, U: np.ndarray, structure: dict, circuit: QuantumCircuit, qudits: List[int]
    ) -> List[Gate]:
        """
        構造に応じた最適な分解
        ...
        if structure["type"] == "general":
            ...
            # 部分空間のみを分解
            return self._sparse_subspace_decomposition(U, structure, circuit, qudits)
    ...
    def _sparse_subspace_decomposition(self, U, structure, circuit, qudits):
        ...
        gates.extend(
            self._move_subspace_to_standard_position(circuit, qudits, active_indices)
        )
        ...
        gates.extend(
            self._decompose_small_unitary(U_sub, circuit, qudits, dim_subspace)
        )

        # ステップ3: 部分空間を元の位置に戻す
        gates.extend(self._move_subspace_back(circuit, qudits, active_indices))
...
class HTransferGateSequence:
    ...
    def add_transfer_evolution(self, i: int, j: int, theta: float):
        ...
        # フレーム復元
        gate6 = gates.R(
            self.circuit, "R_frame_back", j, [0, 1, -np.pi / 2, -np.pi / 2], 3
        )
...
class HTTAGateSequence:
    ...
    def add_tta_evolution(self, i: int, j: int, U_sub_3x3: np.ndarray):
        ...
        self._add_two_level_rotation(
            i, j, levels_i=[0, 1], levels_j=[2, 1], theta=theta1, phi=phi1
        )

        # ステップ3: 第2の2準位回転（例: |11⟩ - |20⟩）
        self._add_two_level_rotation(
            i, j, levels_i=[1, 2], levels_j=[1, 0], theta=theta2, phi=phi2
        )
