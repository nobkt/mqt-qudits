...
def build_H_TTA_matrix(J: float, dim: int = 3) -> np.ndarray:
    ...
    This couples three states in a chain:
    |02⟩ ↔ |11⟩ ↔ |20⟩

    This corresponds to the TTA process:
    |T1⟩_i|T1⟩_j → |S0⟩_i|S1⟩_j (and reverse)
    |T1⟩_i|T1⟩_j → |S1⟩_i|S0⟩_j (and reverse)

    This operates on the 3D subspace {|02⟩, |11⟩, |20⟩}.
    ...
    # |S1⟩_i|S0⟩_j = |2⟩_i|0⟩_j = |20⟩
    idx_20 = 2 * dim + 0  # = 6 for dim=3

    # H_TTA = J(|02⟩⟨11| + |11⟩⟨02| + |11⟩⟨20| + |20⟩⟨11|)
    # This implements the full TTA coupling in the 3D subspace
    H[idx_02, idx_11] = J
    H[idx_11, idx_02] = J
    H[idx_11, idx_20] = J
    H[idx_20, idx_11] = J
...
def build_H_transfer_unitary(V: float, dt: float, hbar: float = 0.6582119569, dim: int = 3) -> np.ndarray:
    ...
    return build_time_evolution_unitary(H, dt, hbar)
...
def test_hamiltonians() -> None:
    ...
    verify_sparse_structure(U_tr)

    U_TTA = build_H_TTA_unitary(J, dt, hbar, dim=3)

    # Verify sparse structure
    verify_sparse_structure(U_TTA)
