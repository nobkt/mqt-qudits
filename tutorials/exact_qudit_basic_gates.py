...
def apply_H_transfer_basic_gates(circuit, qudit_i: int, qudit_j: int, V: float, dt: float, hbar: float) -> None:
...
def apply_H_TTA_basic_gates(circuit, qudit_i: int, qudit_j: int, J: float, dt: float, hbar: float) -> None:
    ...
    **NO approximations, NO "simplified versions", NO heuristics.**
    ...
    if formula_error > 1e-10:
        msg = f"Analytical formula doesn't match expm! Error: {formula_error:.2e}"
        raise ValueError(msg)
    ...
    if unitarity_error_9x9 > 1e-10:
        msg = f"9×9 unitary is not unitary! Error: {unitarity_error_9x9:.2e}"
        raise ValueError(msg)
...
def verify_H_transfer_decomposition(V: float, dt: float, hbar: float, tolerance: float = 1e-12) -> bool:
...
def verify_H_TTA_decomposition(J: float, dt: float, hbar: float, tolerance: float = 1e-10) -> bool:
    ...
    return not formula_error >= tolerance
