...
class OptimizedMQTQuditTimeEvolution:
    ...
    def add_H_transfer_evolution_optimized(self, circuit: QuantumCircuit, dt: float) -> None:
    ...
    def add_H_TTA_evolution_optimized(self, circuit: QuantumCircuit, dt: float) -> None:
        """H_TTAの時間発展を最適化実装.
    ...
    def _add_controlled_rotation_02(self, circuit: QuantumCircuit, i: int, j: int, theta: float, phi: float) -> None:
...
class FullyOptimizedMQTQuditTimeEvolution:
    ...
    def _add_subspace_rotation_02_11(self, circuit: QuantumCircuit, i: int, j: int, theta: float, phi: float) -> None:
        """{|02⟩, |11⟩}部分空間での回転.
    ...
    def _add_subspace_rotation_11_20(self, circuit: QuantumCircuit, i: int, j: int, theta: float, phi: float) -> None:
        """{|11⟩, |20⟩}部分空間での回転."""
        # 同様のパターン
...
class OptimizedSuzukiTrotterMQTQuditSimulator:
    ...
    def __init__(self, params: PhysicalParameters, use_fully_optimized: bool = False) -> None:
        self.params = params
...
class ExactDiagonalizationSolver:
    ...
    def build_hamiltonian(self) -> np.ndarray:
        ...
        for idx in range(self.dim):
            config = index_to_config(idx, self.N, 3)
            energy = sum(self.params.E_T if level == 1 else (self.params.E_S if level == 2 else 0) for level in config)
            H[idx, idx] = energy
    ...
    def simulate(
        ...
        return {"times": times, "populations": populations_history}
...
def compare_qudit_vs_exact(qudit_results: dict, exact_results: dict) -> dict:
    ...
    for q_pop, e_pop in zip(qudit_results["populations"], exact_results["populations"]):
        error = (
            abs(q_pop["N_S0"] - e_pop["N_S0"]) + abs(q_pop["N_T1"] - e_pop["N_T1"]) + abs(q_pop["N_S1"] - e_pop["N_S1"])
        ) / 3.0
...
def test_optimized_gate_construction() -> None:
    """最適化されたゲート構築のテスト."""
    ...
    len(circuit.instructions) - initial_count
    ...
    # CustomTwoを分解
    time_evol.decompose_custom_two_gates(circuit)


def demo_short_optimized_simulation() -> None:
    """短い最適化シミュレーションのデモ."""
    params = PhysicalParameters()
    ...
    simulator.simulate(T_total=T_total, N_steps=N_steps, initial_state_type="all_triplet", track_dynamics=True)
