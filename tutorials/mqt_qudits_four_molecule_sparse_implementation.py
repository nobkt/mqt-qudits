...
class SparseAwareMQTGateGenerator:
        ...
        if subspace_type["type"] == "multi_qudit":
            ...
            for gate in result.gate_sequence.gates:
                ...
                gate_info = {
                    ...
                    if subspace_type["type"] == "single_qudit"
                    else qudit_indices,
                    "params": gate_params,
                }
                gates.append(gate_info)
    ...
    def _convert_global_to_local(
        self, gate_type: str, gate_params: dict, qudit_indices: list[int], dimensions: list[int]
    ) -> tuple[dict, list[int]]:
        ...
        if gate_type in {"R", "Rz", "Rh"} and "level1" in gate_params and "level2" in gate_params:
            ...
            diff_qudits = [qudit_idx for qudit_idx in range(len(dimensions)) if state1[qudit_idx] != state2[qudit_idx]]
    ...
    def _analyze_subspace(self, active_indices: list[int], dimensions: list[int]) -> dict:
        ...
        if len(involved_qudits) == 1:
            ...
            local_levels = sorted({state[qudit_idx] for state in states})
        ...
        # 複数quditにまたがる部分空間
        return {"type": "multi_qudit", "involved_qudits": involved_qudits}
    ...
    def get_compilation_report(self) -> str:
        ...
        if stats["total_gates_estimated_dense"] > 0:
            reduction_rate = (1 - stats["total_gates_sparse"] / stats["total_gates_estimated_dense"]) * 100
...
class SparseAwareMQTQuditTimeEvolution:
    ...
    def add_H0_evolution_gates(self, circuit, dt: float) -> None:
        """H0の時間発展ゲートを回路に追加.
    ...
    def add_H_transfer_evolution_gates(self, circuit, dt: float) -> None:
    ...
    def add_H_TTA_evolution_gates(self, circuit, dt: float) -> None:
    ...
    def _add_gates_to_circuit(self, circuit, gates: list[dict]) -> None:
        """ゲート列を回路に追加.
        ...
        for gate in gates:
            ...
            if gate_type == "VirtRz":
                ...
                circuit.r(qudits[0], [params["level1"], params["level2"], params["theta"], params["phi"]])
                ...
                circuit.rz(qudits[0], [params["level1"], params["level2"], params["phase"]])

            elif gate_type == "Rh":
                # Rh(qudit, [level_a, level_b, theta])
                circuit.rh(qudits[0], [params["level1"], params["level2"], params["theta"]])
    ...
    def decompose_custom_two_gates(self, circuit):
        ...
        """
        if not self.mqt_available:
            msg = "mqt.quditsがインストールされていません"
            raise ImportError(msg)

        # CustomTwoゲートを含むかチェック
        custom_two_gates = [
            (idx, gate) for idx, gate in enumerate(circuit.instructions) if gate.__class__.__name__ == "CustomTwo"
        ]
        ...
        non_custom_two_gates = sum(1 for gate in circuit.instructions if gate.__class__.__name__ != "CustomTwo")
        ...
        total_sparse_gates / len(custom_two_gates) if custom_two_gates else 0
        ...
        total_decomposed_gates / len(custom_two_gates) if custom_two_gates else 0
    ...
    def _decompose_custom_two_exact(self, gate):
        ...
        MQT-QuditsのLogEntQRCEXPassを使用して、CustomTwoゲートを
        基本ゲート（VirtRz, R, CEx, Rz, Rh）に分解します。

        Note: This is a wrapper around _decompose_custom_two_sparse_aware for compatibility.
        ...
        Returns:
            分解後のゲートのリスト
        """
        # Use the existing _decompose_custom_two_sparse_aware method
        # which properly handles LogEntQRCEXPass decomposition
        return self._decompose_custom_two_sparse_aware(gate, gate.parent_circuit)
    ...
    def add_single_trotter_step(self, circuit, dt: float) -> None:
...
class SuzukiTrotterMQTQuditSimulator:
    ...
    def simulate(
        self, T_total: float, N_steps: int, initial_state_type: str = "all_triplet", track_dynamics: bool = True
    ) -> dict:
        """完全なシミュレーションを実行.
        ...
        for step in range(N_steps):
            ...
            # 放射減衰を適用
            current_state = self.apply_radiative_decay_to_statevector(current_state, dt * (step + 1))
    ...
    def simulate_shot_based(
        ...
        for step in range(N_steps):
            ...
            # 状態ベクトルからサンプリング
            probabilities = np.abs(current_state) ** 2
            probabilities /= np.sum(probabilities)  # 正規化
            ...
            if track_dynamics:
                ...
                per_molecule_populations_history.append(
                    self.calculate_per_molecule_populations_from_samples(samples, shots)
                )

            # 進捗表示
            if (step + 1) % max(1, N_steps // 10) == 0 or step == N_steps - 1:
                (step + 1) / N_steps * 100
                populations_history[-1]
    ...
    def build_trotter_circuit(self, dt: float, n_steps: int, initial_state: np.ndarray | None = None):
        """鈴木トロッター回路を構築.
    ...
    def run_simulation(self, dt: float, n_steps: int, initial_state: np.ndarray | None = None):
        """シミュレーションを実行.
        ...
        # バックエンドで実行
        return self.backend.run(circuit, initial_state)
...
class ExactDiagonalizationSolver:
    ...
    def build_hamiltonian(self) -> np.ndarray:
        ...
        for i in range(self.dim):
            ...
            E = sum(self.params.E_T if level == 1 else self.params.E_S if level == 2 else 0 for level in config)
            H[i, i] = E
    ...
    def time_evolve(self, initial_state: np.ndarray, time: float) -> np.ndarray:
        ...
        # 元の基底に戻す
        return self.eigenvectors @ evolved_coeffs
    ...
    def simulate(
        self, T_total: float, N_points: int, initial_state_type: str = "all_triplet", include_decay: bool = False
    ) -> dict:
        """完全なシミュレーション（厳密解）.
        ...
        for t in times:
            ...
            # 放射減衰を適用（オプション）
            state_final = self.apply_radiative_decay(state_evolved, t) if include_decay else state_evolved
