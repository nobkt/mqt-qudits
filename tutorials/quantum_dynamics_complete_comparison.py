...
class ClassicalSuzukiTrotterSimulator:
    ...
    def build_two_site_operator(self, site_i: int, site_j: int, op_i: np.ndarray, op_j: np.ndarray) -> np.ndarray:
        """2サイトの演算子を全空間に拡張."""
        operators = [self.I3] * self.N
    ...
    def build_H_transfer_pair(self, mol_i: int, mol_j: int) -> np.ndarray:
        ...
        return self.params.V * (term + term.conj().T)
    ...
    def build_H_TTA_pair(self, mol_i: int, mol_j: int) -> np.ndarray:
        ...
        return self.params.J * (term1 + term1.conj().T + term2 + term2.conj().T)
    ...
    def simulate(self, T_total: float, N_steps: int, initial_state_type: str = "edge_triplet") -> dict:
...
classical_results = classical_sim.simulate(
    T_total=params.T_total, N_steps=params.N_steps, initial_state_type=params.initial_state_type
)
...
def plot_population_dynamics(results: dict, title: str = "Population Dynamics"):
    ...
    ax.plot(times, N_S0, "b-", linewidth=2.5, label=r"$N_{S_0}$ (Ground singlet)", marker="o", markersize=5, alpha=0.8)
    ax.plot(times, N_T1, "r-", linewidth=2.5, label=r"$N_{T_1}$ (Triplet)", marker="s", markersize=5, alpha=0.8)
    ax.plot(times, N_S1, "g-", linewidth=2.5, label=r"$N_{S_1}$ (Excited singlet)", marker="^", markersize=5, alpha=0.8)
...
# プロット実行
fig_classical = plot_population_dynamics(classical_results, title="Classical Suzuki-Trotter: Population Dynamics")
...
if "per_molecule_populations" in classical_results:
    fig_classical_per_mol = plot_per_molecule_populations(
        classical_results, title="Classical Suzuki-Trotter: Per-Molecule Population Dynamics"
    )
...
class QubitMolecularDynamicsSimulator:
    ...
    def apply_transfer_evolution(self, circuit: QuantumCircuit, mol_i: int, mol_j: int, dt: float) -> None:
    ...
    def apply_TTA_evolution(self, circuit: QuantumCircuit, mol_i: int, mol_j: int, dt: float) -> None:
    ...
    def simulate(
        self, T_total: float, N_steps: int, initial_state_type: str = "edge_triplet", shots: int = 10000
    ) -> dict:
...
# 個体数動態のプロット
fig_qubit = plot_population_dynamics(qubit_results, title="Qubit Implementation: Population Dynamics")
...
for _gate_name, count in sorted(gate_counts.items(), key=operator.itemgetter(1), reverse=True):
    pass
...
if "per_molecule_populations" in qubit_results:
    fig_qubit_per_mol = plot_per_molecule_populations(
        qubit_results, title="Qubit Implementation: Per-Molecule Population Dynamics"
    )
...
qubit_unitary_results = qubit_unitary_sim.simulate(
    T_total=params.T_total, N_steps=params.N_steps, initial_state_type=params.initial_state_type, shots=10000
)
...
from comparison_helpers import (
    compare_gate_counts,
    count_gates_by_type,
    decompose_qiskit_unitary_gates,
    print_gate_statistics,
)
...
# 比較
compare_gate_counts(gates_unitary, gates_decomposed, "UnitaryGate版", "分解後")
...
# #### ノイズモデル
#
# Qiskit Areのノイズモデルを使用して、以下のノイズを含めます：
#
...
try:
    from mqt_qudits_four_molecule_sparse_implementation import PhysicalParameters as MQTPhysicalParameters
...
if qudit_results is not None and "per_molecule_populations" in qudit_results:
    fig_qudit_per_mol = plot_per_molecule_populations(
        qudit_results, title="Qudit Implementation: Per-Molecule Population Dynamics"
    )
...
from comparison_helpers import count_gates_by_type, decompose_qudit_customtwo_gates_to_circuit, print_gate_statistics
...
if qudit_results is not None and "step_circuit" in qudit_results:
    ...
    if num_customtwo > 0:
        ...
        try:
            ...
            # 実際に分解を実行
            decomposed_circuit = decompose_qudit_customtwo_gates_to_circuit(step_circuit_qudit, time_evol)

            # 分解後のゲートをカウント
            gates_decomposed = count_gates_by_type(decomposed_circuit, is_qiskit=False)

            total_decomposed = 0
            for _gate_name, count in sorted(gates_decomposed.items(), key=operator.itemgetter(1), reverse=True):
...
ax1.plot(
    times_qubit, N_T1_qubit, "r-", linewidth=2.5, label=r"$N_{T_1}$ (Noiseless)", marker="s", markersize=5, alpha=0.8
)
ax1.plot(
    times_qubit, N_S1_qubit, "g-", linewidth=2.5, label=r"$N_{S_1}$ (Noiseless)", marker="^", markersize=5, alpha=0.8
)
ax1.plot(
    times_qubit_noisy,
    N_T1_qubit_noisy,
...
if len(times_qudit) > 0 and len(pops_qudit) > 0:
    ...
    ax2.text(
        0.5, 0.5, "Qudit noiseless data not available", ha="center", va="center", transform=ax2.transAxes, fontsize=14
    )
...
# Qubit vs Classical
qubit_errors = calculate_population_error(classical_results, qubit_results, "Classical", "Qubit")


# Qudit vs Classical (if available)
if qudit_results is not None:
    qudit_errors = calculate_population_error(classical_results, qudit_results, "Classical", "Qudit")
...
ax1.semilogy(
    qubit_errors["times"],
    qubit_errors["errors_S0"],
    "b-",
    linewidth=2,
    label=r"$|\Delta N_{S_0}|$",
    marker="o",
    markersize=4,
)
ax1.semilogy(
    qubit_errors["times"],
    qubit_errors["errors_T1"],
    "r-",
    linewidth=2,
    label=r"$|\Delta N_{T_1}|$",
    marker="s",
    markersize=4,
)
ax1.semilogy(
    qubit_errors["times"],
    qubit_errors["errors_S1"],
    "g-",
    linewidth=2,
    label=r"$|\Delta N_{S_1}|$",
    marker="^",
    markersize=4,
)
...
if qudit_errors is not None:
    ...
    ax2 = axes[1]
    ax2.text(0.5, 0.5, "Qudit results not available", ha="center", va="center", fontsize=14)
    ax2.axis("off")
...
ax.set_xlabel("Time (fs)", fontsize=11, fontweight="bold")
ax.set_ylabel("Population", fontsize=11, fontweight="bold")
ax.set_title(f"Qubit Implementation\n{qubit_results['total_gates']} gates", fontsize=12, fontweight="bold")
ax.legend(fontsize=9)
ax.grid(True, alpha=0.3)
ax.set_ylim(0, 4.5)
...
comparison_data = {
    ...
    "回路深さ/ステップ": ["N/A", qubit_results.get("depth_per_step", "N/A"), "N/A"],
    "最大誤差 (N_T1)": [
        "0 (基準)",
    ...
    "実行時間 (秒)": [
        ...
        f"{qudit_results.get('elapsed_time', 'N/A'):.2f}"
        if qudit_results and "elapsed_time" in qudit_results
        else "N/A",
    ],
}
...
plt.title("Comprehensive Comparison of Three Methods", fontsize=14, fontweight="bold", pad=20)
plt.tight_layout()
plt.show()
...
# #### Qubitノイズモデル
#
# Qiskit Areを使用して、超伝導量子ビットに典型的なノイズを実装：
