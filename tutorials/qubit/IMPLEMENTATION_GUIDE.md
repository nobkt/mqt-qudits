...
def test_physical_parameters():
    ...
    # カスタムパラメータ
    params2 = PhysicalParameters(
        N_molecules=6,
        V=[0.1, 0.2, 0.15, 0.1, 0.2]
    )
...
def optimize_circuit(circuit, optimization_level=3):
    ...
    print(f"Reduction: {100*(1 - sum(optimized_circuit.count_ops().values())/sum(circuit.count_ops().values())):.1f}%")
