#!/usr/bin/env python3
"""Iteration 27 verification script.

Checks:
- Issue HH fix + Issue II fix (g_eph=0 API consistency): QuditGKSLCircuitBosonSimulator._precompute_unitaries now
  uses build_H_total_boson + expm for the Hamiltonian half-step, matching
  QuditGKSLBosonSimulator (instead of first-order Trotter product).
- Regression: All iteration 25 checks (95 checks, Sections 1-16) still pass.
- New: Section 17 adds checks for Issue II (g_eph=0 API consistency).
...
def main() -> None:
    ...
    # ================================================================
    # Section 16: Issue HH - Circuit boson Hamiltonian uses exact expm (NEW)
    # ================================================================
    print("\n=== Section 16: Issue HH - Circuit boson Hamiltonian fix ===")

    # a) U_H_half should match between circuit and DM
    # sim_circ_boson already has _U_H_half from simulate() call in Section 11
    # sim_qudit_boson._U_H_half from Section 13's simulate() call
    u_h_diff = float(np.linalg.norm(sim_circ_boson._U_H_half - sim_qudit_boson._U_H_half))
    add("circuit_boson_U_H_half_matches_dm",
        u_h_diff < 1e-14,
        f"||U_H_half_circuit - U_H_half_dm||_F = {u_h_diff:.2e} (should be < 1e-14)")

    # b) Full rho comparison after 2 steps
    # Manually run Trotter steps to get full density matrix (not partial traced)
    sim_dm_full = QuditGKSLBosonSimulator(params_boson)
    sim_circ_full = QuditGKSLCircuitBosonSimulator(params_boson)
    dt_check = 0.1 / 2
    sim_dm_full._precompute_unitaries(dt_check)
    sim_circ_full._precompute_unitaries(dt_check)
    rho_dm_full = sim_dm_full.prepare_initial_state("edge_triplet")
    rho_circ_full = sim_circ_full.prepare_initial_state("edge_triplet")
    for _ in range(2):
        rho_dm_full = sim_dm_full._trotter_step(rho_dm_full)
        rho_circ_full = sim_circ_full._trotter_step(rho_circ_full)
    full_rho_diff = float(np.linalg.norm(rho_dm_full - rho_circ_full))
    add("circuit_boson_full_rho_matches_dm",
        full_rho_diff < 1e-12,
        f"||rho_full_circuit - rho_full_dm||_F (2 steps) = {full_rho_diff:.2e} (should be < 1e-12)")

    # c) Phonon reduced rho comparison
    dim_el_check = sim_dm_full.dim_el
    dim_ph_check = sim_dm_full.dim_ph
    rho_ph_dm = np.trace(rho_dm_full.reshape(dim_el_check, dim_ph_check, dim_el_check, dim_ph_check), axis1=0, axis2=2)
    rho_ph_circ = np.trace(rho_circ_full.reshape(dim_el_check, dim_ph_check, dim_el_check, dim_ph_check), axis1=0, axis2=2)
    phonon_rho_diff = float(np.linalg.norm(rho_ph_dm - rho_ph_circ))
    add("circuit_boson_phonon_rho_matches_dm",
        phonon_rho_diff < 1e-12,
        f"||rho_phonon_circuit - rho_phonon_dm||_F (2 steps) = {phonon_rho_diff:.2e} (should be < 1e-12)")

    # d) Full rho comparison after 20 steps
    sim_dm_perf2 = QuditGKSLBosonSimulator(params_boson)
    sim_circ_perf2 = QuditGKSLCircuitBosonSimulator(params_boson)
    dt_perf2 = 1.0 / 20
    sim_dm_perf2._precompute_unitaries(dt_perf2)
    sim_circ_perf2._precompute_unitaries(dt_perf2)
    rho_dm_20 = sim_dm_perf2.prepare_initial_state("edge_triplet")
    rho_circ_20 = sim_circ_perf2.prepare_initial_state("edge_triplet")
    for _ in range(20):
        rho_dm_20 = sim_dm_perf2._trotter_step(rho_dm_20)
        rho_circ_20 = sim_circ_perf2._trotter_step(rho_circ_20)
    full_rho_20_diff = float(np.linalg.norm(rho_dm_20 - rho_circ_20))
    add("circuit_boson_full_rho_20step_matches_dm",
        full_rho_20_diff < 1e-10,
        f"||rho_full_circuit - rho_full_dm||_F (20 steps) = {full_rho_20_diff:.2e} (should be < 1e-10)")

    # e) Source code verification: _precompute_unitaries uses build_H_total_boson
    precompute_start = circ_boson_src.find("def _precompute_unitaries(")
    if precompute_start >= 0:
        next_def = circ_boson_src.find("\n    def ", precompute_start + 1)
        precompute_body = circ_boson_src[precompute_start:next_def] if next_def > 0 else circ_boson_src[precompute_start:]
        uses_h_total = "build_H_total_boson" in precompute_body
        not_uses_trotter = "_compute_hamiltonian_unitary" not in precompute_body
        add("circuit_boson_uses_build_H_total",
            uses_h_total and not_uses_trotter,
            f"_precompute_unitaries should use build_H_total_boson (found={uses_h_total}) "
            f"and NOT _compute_hamiltonian_unitary (absent={not_uses_trotter})")
    else:
        add("circuit_boson_uses_build_H_total", False, "Could not find _precompute_unitaries")

    # f) Source code: _precompute_unitaries uses expm
    if precompute_start >= 0:
        uses_expm = "expm" in precompute_body
        add("circuit_boson_uses_expm_for_H",
            uses_expm,
            f"_precompute_unitaries should use expm for Hamiltonian (found={uses_expm})")
    else:
        add("circuit_boson_uses_expm_for_H", False, "Could not find _precompute_unitaries")


    # ================================================================
    # Section 17: Issue II - g_eph=0 API consistency (new in iteration 27)
    # ================================================================
    print("\n=== Section 17: Issue II - g_eph=0 API consistency ===")

    params_geph0 = GKSLPhysicalParameters(
        N_molecules=2, with_boson=True, n_max=1, omega_ph=0.15, g_eph=0.0,
    )
    result_geph0 = QuditGKSLCircuitBosonSimulator(params_geph0).simulate(
        t_max=10.0, n_steps=2, initial_state="edge_triplet",
    )

    # a) n_total_qudits key present
    add("geph0_has_n_total_qudits",
        "n_total_qudits" in result_geph0,
        f"g_eph=0 result should have 'n_total_qudits' key (present={'n_total_qudits' in result_geph0})")

    # b) n_phonon_qudits key present and = 0
    has_n_ph = "n_phonon_qudits" in result_geph0
    n_ph_val = result_geph0.get("n_phonon_qudits", "MISSING")
    add("geph0_n_phonon_zero",
        has_n_ph and n_ph_val == 0,
        f"g_eph=0 n_phonon_qudits should be 0 (value={n_ph_val})")

    # c) dim_total key present
    add("geph0_has_dim_total",
        "dim_total" in result_geph0,
        f"g_eph=0 result should have 'dim_total' key (present={'dim_total' in result_geph0})")

    # d) estimated_gates_per_step key present
    add("geph0_has_estimated_gps",
        "estimated_gates_per_step" in result_geph0,
        f"g_eph=0 result should have 'estimated_gates_per_step' key "
        f"(present={'estimated_gates_per_step' in result_geph0})")

    # e) total_estimated_gates key present
    add("geph0_has_total_estimated_gates",
        "total_estimated_gates" in result_geph0,
        f"g_eph=0 result should have 'total_estimated_gates' key "
        f"(present={'total_estimated_gates' in result_geph0})")

    # f) n_total_qudits = n_system + n_phonon + n_ancilla
    if "n_total_qudits" in result_geph0:
        n_sys = result_geph0.get("n_system_qudits", 0)
        n_ph = result_geph0.get("n_phonon_qudits", 0)
        n_anc = result_geph0.get("n_ancilla_qudits", 0)
        expected_total = n_sys + n_ph + n_anc
        actual_total = result_geph0["n_total_qudits"]
        add("geph0_n_total_sum_correct",
            actual_total == expected_total,
            f"g_eph=0 n_total={actual_total} should equal "
            f"n_sys({n_sys})+n_ph({n_ph})+n_anc({n_anc})={expected_total}")
    else:
        add("geph0_n_total_sum_correct", False, "n_total_qudits key missing")

    # g) rho_final shape is (9, 9) (electronic-only)
    rho_shape = result_geph0["rho_final"].shape
    add("geph0_rho_final_shape",
        rho_shape == (9, 9),
        f"g_eph=0 rho_final shape should be (9,9), got {rho_shape}")

    # h) method string indicates exact reduction
    method_str = result_geph0.get("method", "")
    add("geph0_method_indicates_reduction",
        "exact reduction" in method_str or "g_eph=0" in method_str,
        f"g_eph=0 method should indicate reduction: '{method_str}'")

    # i) trace conservation
    trace_geph0 = float(np.real(np.trace(result_geph0["rho_final"])))
    add("geph0_trace_conservation",
        abs(trace_geph0 - 1.0) < 1e-12,
        f"g_eph=0 Tr(rho_final) = {trace_geph0:.10f}")
    ...
    out_file = os.path.join(
        out_dir, f"iteration27_api_consistency_{results['timestamp']}.json"
    )
