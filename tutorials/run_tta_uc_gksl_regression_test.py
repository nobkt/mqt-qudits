#!/usr/bin/env python3
"""TTA-UC GKSL stable regression test.

This script performs all physics and mathematics verification checks for the
GKSL-Lindblad quantum dynamics simulators, WITHOUT iteration-count-dependent
checks that create a self-referential infinite loop.

Checks included (31 checks):
  - Checks 1-6: Convergence analysis, density matrix quality, notebook values
  - Check 9: Cell 27 n_steps=200 value
  - Checks 10-30: Particle conservation, shot/DM consistency, boson simulators
  - Checks 32-34: Attribute checks, error coefficient stability, Stinespring O(dt²)

Checks REMOVED (3 checks that caused infinite loop):
  - Check 7: Cell 25 iteration count string (self-referential)
  - Check 8: Cell 39 iteration count string (self-referential)
  - Check 31: Cell 25/39 iteration count verification (self-referential)
...
def check_documentation_fixes(notebook_path: str) -> dict:
    """Check that notebook documentation values are correct.

    Only checks content-based values (Cell 27 precision table),
    NOT iteration count strings (which caused the infinite loop).
    """
    with open(notebook_path) as f:
        nb = json.load(f)
...
def main() -> None:
    ...
    results: dict = {
        ...
        "timestamp": timestamp,
        "description": "Stable regression test (31 physics/math checks, no iteration-count checks)",
        "params": {
            "t_max": t_max,
    ...
    print("=" * 70)
    print("TTA-UC GKSL Stable Regression Test (31 checks)")
    print("=" * 70)
    ...
    # ===================================================================
    # Part 4: Documentation value validation (check 9 only)
    # Checks 7/8 (iteration count strings) REMOVED — caused infinite loop
    # ===================================================================
    print("\n--- Part 4: Documentation value validation (check 9) ---")
    ...
    # ===================================================================
    # Check 31 REMOVED — iteration count string verification caused
    # self-referential infinite loop. See root cause analysis in
    # developing/検証結果分析_iteration46_根本原因分析.md
    # ===================================================================
    ...
    all_pass = all([
        ...
        boson_qd_qb_ok,
        boson_baseline_ok,
        # Check 31 REMOVED (self-referential iteration count)
        no_stines_ok,
        # Check 33-34
        coeff_stable,
        stine_channel_ok,
    ...
    # Save results
    # ===================================================================
    json_path = results_dir / f"regression_test_{timestamp}.json"
    with open(json_path, "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False, default=str)
    print(f"\nJSON results saved to: {json_path}")

    md_path = results_dir / f"regression_test_{timestamp}.md"
    md_lines = [
        f"# TTA-UC GKSL Regression Test Results\n",
        f"\n",
        f"- 実行日時: {timestamp}\n",
        f"- 検証対象: 31チェック（物理・数学チェックのみ、イテレーション回数チェック除外）\n",
        f"\n",
        f"## 収束解析結果（4分子系）\n",
        f"\n",
    ...
    md_lines.extend([
        ...
        f"\n",
        f"**注**: チェック 7, 8, 31（イテレーション回数文字列の検証）は自己参照的無限ループの原因のため除外。\n",
        f"詳細は `developing/検証結果分析_iteration46_根本原因分析.md` を参照。\n",
        f"\n",
