#!/usr/bin/env python3
"""TTA-UC GKSL stable verification script.

This is the stable (non-iteration-specific) verification script that validates
...
counter checks that caused the infinite verification loop.
...
def main() -> None:
    ...
    results: dict = {
        "iteration": "stable",
        "timestamp": timestamp,
        "description": "Stable regression tests (checks 1-31, iteration-counter checks removed)",
        "params": {
            "t_max": t_max,
    ...
    print("=" * 70)
    print("TTA-UC GKSL Stable Verification (regression tests)")
    print("=" * 70)
    ...
    # ===================================================================
    print("\n--- Part 26: Cell 25/39 iteration-count verification (check 31 - REMOVED) ---")

    # Self-referential iteration counter check removed.
    # This check caused the infinite verification loop:
    #   Each iteration checked for "N回" and updated it to "N+1回",
    #   which required another iteration to verify... ad infinitum.
    # See developing/検証結果分析_iteration45_根本原因分析.md for details.

    cell_stable_ok = True  # Always pass - no longer self-referential
    ...
    # Save results
    # ===================================================================
    json_path = results_dir / f"stable_verification_{timestamp}.json"
    with open(json_path, "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False, default=str)
    print(f"\nJSON results saved to: {json_path}")

    md_path = results_dir / f"stable_verification_{timestamp}.md"
    md_lines = [
        f"# TTA-UC GKSL 安定検証結果\n",
        f"\n",
        f"- 実行日時: {timestamp}\n",
