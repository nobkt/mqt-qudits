#!/usr/bin/env python3
"""TTA-UC GKSL iteration 19 verification: trace distance & classical Trotter.

Fixes issues identified in iteration 18 analysis:
  - Test A-D: unchanged from iteration 18
  - Test E (modified): replace invalid subtraction-based error decomposition
    with proper trace distance measurement and classical Trotter comparison.
    Add convergence rate analysis for both fidelity and trace distance.
    Verify 1-F ≈ T² relationship (Fuchs-van de Graaf).
...
# =========================================================================
# Test E: Exact Liouvillian comparison (MODIFIED in iteration 19)
# =========================================================================
...
def test_exact_liouvillian_comparison(params: GKSLPhysicalParameters) -> dict:
    ...
    MODIFIED from iteration 18:
    - Add trace distance alongside fidelity for proper error characterization
    - Add classical Trotter reference for proper error source separation
    - Remove invalid subtraction-based "Stinespring approximation" decomposition
    - Verify 1-F ≈ T² relationship (Fuchs-van de Graaf)
    - Compute convergence rates for both metrics
    """
    print("\n" + "=" * 70)
    ...
    print(f"\n  Part 2: Three-way convergence: ST vs Exact vs Classical Trotter")
    print(
        f"  {'n_steps':>7} | {'dt':>5} | "
        f"{'1-F(ST,ex)':>11} | {'T(ST,ex)':>11} | "
        f"{'1-F(CT,ex)':>11} | {'T(CT,ex)':>11} | "
        f"{'1-F(ST,CT)':>11} | {'T(ST,CT)':>11} | "
        f"{'(1-F)/T²':>9}"
    )
    print("  " + "-" * 110)
    ...
    for ns in step_counts:
        ...
        infid_ct_ex = 1.0 - f_ct_ex

        # Metrics: ST vs ClassicalTrotter
        f_st_ct = quantum_fidelity(rho_ct, rho_st)
        t_st_ct = trace_distance(rho_ct, rho_st)
        infid_st_ct = 1.0 - f_st_ct

        # Verify 1-F ≈ T²
        ratio_1mF_T2 = infid_st_ex / (t_st_ex ** 2) if t_st_ex > 1e-15 else float("nan")
        ...
        entry = {
            ...
            # 1-F vs T² ratio
            "ratio_1mF_over_T2": ratio_1mF_T2,
            # Populations
            "N_S1_ST": pop_st["N_S1"],
            "N_S1_CT": pop_ct["N_S1"],
        ...
        print(
            ...
            f"{infid_st_ct:11.4e} | {t_st_ct:11.4e} | "
            f"{ratio_1mF_T2:9.4f}"
        )
    ...
    if len(convergence_data) >= 2:
        ...
        for k in range(len(rates_infid_st_ex)):
            ...
            print(
                ...
                f"{rates_infid_st_ex[k]:10.2f} | {rates_tdist_st_ex[k]:10.2f}"
            )
        ...
        for k in range(len(rates_infid_ct_ex)):
            ...
            print(
                f"  {dt_prev:.1f} → {dt_curr:.1f}      | "
                f"{rates_infid_ct_ex[k]:10.2f} | {rates_tdist_ct_ex[k]:10.2f}"
            )
        avg_infid_ct_ex = np.nanmean(rates_infid_ct_ex)
        avg_tdist_ct_ex = np.nanmean(rates_tdist_ct_ex)
        print(f"  Average: {avg_infid_ct_ex:10.2f} | {avg_tdist_ct_ex:10.2f}")
        ...
        for k in range(len(rates_infid_st_ct)):
            ...
            print(
                f"  {dt_prev:.1f} → {dt_curr:.1f}      | "
                f"{rates_infid_st_ct[k]:10.2f} | {rates_tdist_st_ct[k]:10.2f}"
            )
        avg_infid_st_ct = np.nanmean(rates_infid_st_ct)
        avg_tdist_st_ct = np.nanmean(rates_tdist_st_ct)
        print(f"  Average: {avg_infid_st_ct:10.2f} | {avg_tdist_st_ct:10.2f}")

    else:
        avg_infid_st_ex = avg_tdist_st_ex = float("nan")
        avg_infid_ct_ex = avg_tdist_ct_ex = float("nan")
        avg_infid_st_ct = avg_tdist_st_ct = float("nan")

    # --- Part 4: Step-by-step comparison at n_steps=10 ---
    print(f"\n  Part 4: Step-by-step ST vs Exact at n_steps=10 (with trace distance)")
    ...
    print(
        f"  {'Step':>4} | {'1-F':>10} | {'T_dist':>10} | {'(1-F)/T²':>9} | "
        f"{'Tr_ST':>10} | {'min_eig':>10} | {'N_S1_ST':>10} | {'N_S1_exact':>10}"
    )
    print("  " + "-" * 95)
    ...
    for s in range(11):
        ...
        ratio_s = infid_s / (t_vs_exact_s ** 2) if t_vs_exact_s > 1e-15 else float("nan")
        ...
        print(
            ...
            f"{ratio_s:9.4f} | {tr_s:10.8f} | {min_eig_s:10.2e} | "
            f"{pop_st_s['N_S1']:10.8f} | {pop_exact_s['N_S1']:10.8f}"
        )
...
def main() -> int:
    ...
    print("=" * 70)
    print("TTA-UC GKSL Iteration 19: Trace Distance & Classical Trotter")
    ...
    results: dict = {
        "timestamp": timestamp,
        "iteration": 19,
        "purpose": "Fix iteration 18 issues: add trace distance measurement, "
                   "classical Trotter reference, proper convergence analysis. "
                   "Remove invalid subtraction-based error decomposition.",
        "tests": [],
    }
    ...
    # Write JSON report
    # ===================================================================
    json_path = output_dir / f"iteration19_trace_distance_{timestamp}.json"
    ...
    md_lines.extend([
        ...
        "",
        "| n_steps | dt | 1-F(ST,ex) | T(ST,ex) | 1-F(CT,ex) | T(CT,ex) | "
        "1-F(ST,CT) | T(ST,CT) | (1-F)/T² |",
        "|---------|-----|-----------|---------|-----------|---------|"
        "-----------|---------|---------|",
    ])
    for cd in te["convergence_data"]:
        ratio_str = f"{cd['ratio_1mF_over_T2']:.4f}" if not np.isnan(cd.get("ratio_1mF_over_T2", float("nan"))) else "—"
        md_lines.append(
            f"| {cd['n_steps']} | {cd['dt']:.2f} | "
            f"{cd['infid_ST_vs_exact']:.4e} | {cd['T_ST_vs_exact']:.4e} | "
            f"{cd['infid_CT_vs_exact']:.4e} | {cd['T_CT_vs_exact']:.4e} | "
            f"{cd['infid_ST_vs_CT']:.4e} | {cd['T_ST_vs_CT']:.4e} | "
            f"{ratio_str} |"
        )
    ...
    md_lines.extend([
        ...
        f"**平均 Rate(T): {te.get('avg_rate_tdist_ST_vs_exact', float('nan')):.2f}**",
        "",
        "#### 古典 Trotter vs Exact",
        "",
        "| dt 範囲 | Rate(1-F) | Rate(T) |",
    ...
    for sd in te["step_detail_n10"]:
        ratio_str = f"{sd['ratio_1mF_over_T2']:.4f}" if not np.isnan(sd.get("ratio_1mF_over_T2", float("nan"))) else "—"
        md_lines.append(
            f"| {sd['step']} | {sd['infidelity']:.4e} | {sd['trace_dist_vs_exact']:.4e} | "
            f"{ratio_str} | {sd['trace_ST']:.8f} | {sd['min_eigenvalue_ST']:.2e} | "
    ...
    md_path = output_dir / f"iteration19_trace_distance_{timestamp}.md"
    md_path.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
    ...
    print(f"  Test E: avg convergence rate ST vs Exact: "
          ...
          f"T={te.get('avg_rate_tdist_ST_vs_exact', float('nan')):.2f}")
    print(f"  Test E: avg convergence rate CT vs Exact: "
          f"1-F={te.get('avg_rate_infid_CT_vs_exact', float('nan')):.2f}, "
          f"T={te.get('avg_rate_tdist_CT_vs_exact', float('nan')):.2f}")
