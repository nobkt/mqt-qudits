#!/usr/bin/env python3
"""TTA-UC GKSL iteration 21 verification: Error decomposition via Classical Product Trotter.

Changes from iteration 20:
  - Test A-D: unchanged from iteration 20
  - Test E (modified):
    * Add "Classical Product Trotter" (CPT) comparison target:
      applies individual Lindblad dissipator exponentials sequentially
      in superoperator space (no Stinespring), with Strang splitting for H-D.
    * New function _build_individual_dissipator_superoperators()
    * New function _classical_product_trotter_simulate()
    * 6 comparison pairs: ST vs Exact, CT vs Exact, CPT vs Exact,
      ST vs CT, ST vs CPT, CPT vs CT
    * ST vs CPT isolates the Stinespring approximation error
    * CPT vs CT isolates the Lie-Trotter product error
    * Note: iteration 21 uses the FIXED simulator (symmetric Lindblad product),
      so ST convergence may improve compared to iteration 20.
...
def test_exact_liouvillian_comparison(params: GKSLPhysicalParameters) -> dict:
    """Compare Stinespring+Trotter against exact GKSL, classical Trotter, and classical product Trotter.

    MODIFIED from iteration 20:
    - Add "Classical Product Trotter" (CPT) comparison target
    - 6 comparison pairs: ST vs Exact, CT vs Exact, CPT vs Exact,
      ST vs CT, ST vs CPT, CPT vs CT
    - ST vs CPT isolates the Stinespring approximation error
    ...
    """
    print("\n" + "=" * 70)
    print("Test E: Exact Liouvillian Comparison (ST vs Exact vs CT vs CPT)")
    print("       [iter 21: +Classical Product Trotter, error decomposition]")
    print("       [NOTE: ST uses FIXED symmetric Lindblad product ordering]")
    print("=" * 70)
    ...
    print("  Building dissipator superoperators for CT and CPT...", flush=True)
    ...
    # --- Part 2: Four-way convergence comparison ---
    print(f"\n  Part 2: Four-way convergence (primary metric: trace distance)")
    print(
        f"  {'n_steps':>7} | {'dt':>5} | "
        f"{'T(ST,ex)':>11} | {'T(CT,ex)':>11} | {'T(CPT,ex)':>11} | "
        f"{'T(ST,CT)':>11} | {'T(ST,CPT)':>11} | {'T(CPT,CT)':>11}"
    )
    print("  " + "-" * 100)
    ...
    for ns in step_counts:
        ...
        fidelity_clipped = (
            ...
            or fidelity_clipped_st_ct or fidelity_clipped_st_cpt or fidelity_clipped_cpt_ct
        )
        ...
        entry = {
            ...
            # ST vs CPT (isolates Stinespring error)
            ...
            # CPT vs CT (isolates Lie-Trotter product error)
    ...
    # Mapping from rate key to convergence_data key
    pair_map = {
    ...
    if len(convergence_data) >= 2:
        ...
        pair_labels = [
            ...
            ("st_cpt", "ST vs Classical Product Trotter (isolates Stinespring error)"),
            ("cpt_ct", "CPT vs CT (isolates Lie-Trotter product error)"),
        ]
    ...
    print(f"  {'n_steps':>7} | {'dt':>5} | {'N_S1(ST)':>12} | {'N_S1(CT)':>12} | "
          f"{'N_S1(CPT)':>12} | {'N_S1(exact)':>12} | {'|Δ|(ST)':>12} | "
          f"{'|Δ|(CT)':>12} | {'|Δ|(CPT)':>12}")
    print("  " + "-" * 120)
    ...
    for cd in convergence_data:
        ...
        print(
            ...
            f"{cd['N_S1_CT']:12.8f} | {cd['N_S1_CPT']:12.8f} | "
            f"{cd['N_S1_exact']:12.8f} | "
            f"{diff_st:12.4e} | {diff_ct:12.4e} | {diff_cpt:12.4e}"
        )
    ...
    if len(convergence_data) >= 2:
        ...
        print(f"  {'dt_range':>15} | {'Rate(ST)':>10} | {'Rate(CT)':>10} | {'Rate(CPT)':>10}")
        print("  " + "-" * 55)
        for k in range(1, len(convergence_data)):
            dt_prev = convergence_data[k-1]["dt"]
            dt_curr = convergence_data[k]["dt"]
            ...
            print(
                ...
                f"{rate_ct:10.2f} | {rate_cpt:10.2f}"
            )
        print(
            f"  Average: {np.nanmean(rates_ns1_st):10.2f} | "
            f"{np.nanmean(rates_ns1_ct):10.2f} | {np.nanmean(rates_ns1_cpt):10.2f}"
        )
...
def main() -> int:
    ...
    print("=" * 70)
    print("TTA-UC GKSL Iteration 21: Error Decomposition via Classical Product Trotter")
    print(f"Timestamp: {timestamp}")
    print("NOTE: ST simulator uses FIXED symmetric Lindblad product ordering")
    print("=" * 70)

    results: dict = {
        "timestamp": timestamp,
        "iteration": 21,
        "purpose": "Error decomposition: add Classical Product Trotter (CPT) to isolate "
                   "Stinespring approximation error (ST vs CPT) from Lie-Trotter product "
                   "error (CPT vs CT). Iteration 21 uses the FIXED simulator with symmetric "
                   "(palindromic) Lindblad channel ordering, which eliminates the sequential "
                   "Lie-Trotter product ordering error source (b).",
        "simulator_note": "ST uses symmetric Lindblad product ordering (fixed in iter 21). "
                          "ST convergence may improve compared to iteration 20.",
        "tests": [],
    }
    ...
    # Test E: Exact Liouvillian comparison (MODIFIED with CPT)
    ...
    # Write JSON report
    # ===================================================================
    json_path = output_dir / f"iteration21_error_decomposition_{timestamp}.json"
    ...
    # Test E summary (MODIFIED for iteration 21)
    md_lines.extend([
    ...
    for cd in te["convergence_data"]:
        md_lines.append(
            ...
            f"{cd['T_CPT_vs_exact']:.4e} | {cd['T_ST_vs_CT']:.4e} | "
            f"{cd['T_ST_vs_CPT']:.4e} | {cd['T_CPT_vs_CT']:.4e} | "
            f"{cd['purity_ST']:.6f} | {cd['purity_CT']:.6f} | {cd['purity_CPT']:.6f} |"
        )
    ...
    pair_labels_md = [
        ...
        ("cpt_ex", "Classical Product Trotter vs Exact"),
        ("st_ct", "ST vs Classical Trotter"),
        ("st_cpt", "ST vs CPT (Stinespring 誤差)"),
        ("cpt_ct", "CPT vs CT (Lie-Trotter 積誤差)"),
    ]
    ...
    for cd in te["convergence_data"]:
        ...
        md_lines.append(
            ...
            f"{cd['N_S1_CT']:.8f} | {cd['N_S1_CPT']:.8f} | "
            f"{cd['N_S1_exact']:.8f} | "
            f"{diff_st:.4e} | {diff_ct:.4e} | {diff_cpt:.4e} |"
        )

    md_lines.append("")

    md_path = output_dir / f"iteration21_error_decomposition_{timestamp}.md"
    md_path.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
    ...
    print(f"  Test E: avg convergence rate ST vs Exact: "
          ...
          f"d_B={te.get('avg_rate_bures_st_ex', float('nan')):.2f}")
    print(f"  Test E: avg convergence rate CT vs Exact: "
          f"T(primary)={te.get('avg_rate_tdist_ct_ex', float('nan')):.2f}, "
          f"1-F={te.get('avg_rate_infid_ct_ex', float('nan')):.2f}, "
          f"d_B={te.get('avg_rate_bures_ct_ex', float('nan')):.2f}")
    print(f"  Test E: avg convergence rate CPT vs Exact: "
          f"T(primary)={te.get('avg_rate_tdist_cpt_ex', float('nan')):.2f}, "
          f"1-F={te.get('avg_rate_infid_cpt_ex', float('nan')):.2f}, "
          f"d_B={te.get('avg_rate_bures_cpt_ex', float('nan')):.2f}")
    print(f"  Test E: --- Error decomposition ---")
    print(f"  Test E: avg rate ST vs CPT (Stinespring error): "
          f"T={te.get('avg_rate_tdist_st_cpt', float('nan')):.2f}")
    print(f"  Test E: avg rate CPT vs CT (Lie-Trotter product error): "
