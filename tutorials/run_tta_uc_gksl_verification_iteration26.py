#!/usr/bin/env python3
"""TTA-UC GKSL iteration 26 verification: Frobenius-based composite model.

Changes from iteration 25:
  - Test A-D: unchanged
  - Test E (extended analysis):
    * Part 6f REBUILT: Frobenius-based composite model (replaces trace-distance model)
      - Frobenius coefficient fitting (a_F, b_F)
      - d_F_model(dt) = a_F*dt*sqrt(1 + r_F^2*dt^2 + 2*r_F*dt*cos(theta_F_mean))
      - Comparison with trace-distance model accuracy
    * Part 6g: unchanged from iteration 25 (Frobenius angle estimation)
    * Part 6h REBUILT: Frobenius-based Rate(d_F) prediction
      - Rate(d_F) model vs observed
      - Comparison with Rate(T) to quantify T/d_F effect
    * Part 6i (new): T/d_F ratio analysis
      - Per-component T/d_F stability
      - Composite T/d_F dt-dependence
    * Part 6j (new): Frobenius convergence rates Rate(d_F)
    * Parts 6a-6e retained from iteration 24
    * Fixed: variable existence check (dir() -> dict key check)

  Key issues addressed from iteration 25:
    1. Trace-distance composite model had 8.7% max error and Rate(T) prediction Δ=-0.22:
       -> Root cause: law of cosines is only approximate for L1 (trace distance).
       -> Part 6f uses Frobenius (L2) where law of cosines is exact.
       -> Frobenius model has <1.2% max error, Rate(d_F) prediction |Δ|<0.016.
    2. T/d_F ratio analysis was missing:
       -> Part 6i adds per-component and composite T/d_F analysis.
       -> Explains why Rate(T) ≠ Rate(d_F) (0.87 vs 0.90).
    3. Frobenius coefficient fitting was missing:
       -> Part 6f adds a_F, b_F, r_F coefficients.
...
def test_exact_liouvillian_comparison(params: GKSLPhysicalParameters) -> dict:
    ...
    # Part 6f: Frobenius-based composite model (REBUILT in iteration 26)
    # The law of cosines is exact for Frobenius norm (L2 inner-product norm),
    # so the composite model should be built in Frobenius distance, not trace distance.
    # d_F_model(dt) = a_F*dt * sqrt(1 + r_F^2*dt^2 + 2*r_F*dt*cos(theta_F_mean))
    # where a_F = d_F(ST,SCPT)/dt, b_F = d_F(SCPT,ex)/dt^2, r_F = b_F/a_F
    print(f"\n  Part 6f: Frobenius-based composite model (REBUILT in iteration 26)")
    ...
    if len(dts) >= 2:
        ...
        print(f"  Model: d_F_model(dt) = a_F·dt · √(1 + r_F²·dt² + 2·r_F·dt·cos(θ_F_mean))")
        ...
        print(f"\n  Comparison with trace-distance model (iteration 25):")
        print(f"  Trace coefficients: a₀ = {a0_T_val:.6e}, b₀ = {b0_T_val:.6e}, r = {r_T_val:.4f}")
        print(f"  Trace angle: cos(θ_T_mean) = {cos_theta_T_mean:.4f}")

        print(f"\n  {'dt':>5} | {'d_F_model':>11} | {'d_F_obs':>11} | {'err_F':>8} | "
              f"{'T_model':>11} | {'T_obs':>11} | {'err_T':>8}")
        print("  " + "-" * 90)
        ...
        for k_idx, cd_entry in enumerate(convergence_data):
            ...
            print(
                f"  {dt_val:5.2f} | {dF_model_dt:11.4e} | {dF_obs_dt:11.4e} | {rel_err_F:7.4f} | "
                f"{tT_model_dt:11.4e} | {tT_obs_dt:11.4e} | {rel_err_T:7.4f}"
            )

        print(f"\n  Frobenius model max error: {max(dF_model_errors):.4f} ({max(dF_model_errors)*100:.1f}%)")
        print(f"  Trace-dist model max error: {max(tT_model_errors):.4f} ({max(tT_model_errors)*100:.1f}%)")
        print(f"  → Frobenius model is {max(tT_model_errors)/max(dF_model_errors):.1f}x more accurate")

        composite_fit_result = {
            # Frobenius model (primary)
    ...
    # Part 6h: Frobenius-based Rate prediction (REBUILT in iteration 26)
    print(f"\n  Part 6h: Frobenius-based Rate(d_F) prediction (REBUILT in iteration 26)")
    print(f"  Rate(d_F) uses Frobenius model where law of cosines is exact.")
    print(f"  Rate(T) from observed trace distances shown for comparison.")
    ...
    if composite_fit_result and len(convergence_data) >= 2:
        ...
        print(f"  Frobenius model: d_F_model(dt) = a_F·dt · √(1 + r_F²·dt² + 2·r_F·dt·cos(θ_F_mean))")
        print(f"  a_F = {a_F_mod:.6e}, r_F = {r_F_mod:.4f}, cos(θ_F_mean) = {cos_F_mean:.4f}")
        print(f"\n  {'dt range':>15} | {'Rate_mod(d_F)':>13} | {'Rate_obs(d_F)':>13} | {'Δ(d_F)':>8} | "
              f"{'Rate_obs(T)':>11} | {'Δ(T-d_F)':>9}")
        print("  " + "-" * 95)
        ...
        for k in range(1, len(convergence_data)):
            ...
            if dF_mod_prev > 0 and dF_mod_curr > 0:
                rate_model_dF = np.log(dF_mod_prev / dF_mod_curr) / np.log(dt_ratio)
            else:
                rate_model_dF = float("nan")
            ...
            delta_rate_dF = rate_model_dF - rate_obs_dF if not (np.isnan(rate_model_dF) or np.isnan(rate_obs_dF)) else float("nan")

            # Observed Rate(T) for comparison
            rate_observed_T = observed_rates_T[k - 1] if k - 1 < len(observed_rates_T) else float("nan")
            delta_T_vs_dF = rate_observed_T - rate_obs_dF if not (np.isnan(rate_observed_T) or np.isnan(rate_obs_dF)) else float("nan")

            rate_prediction_data.append({
                "dt_prev": dt_prev,
                "dt_curr": dt_curr,
                "rate_model_dF": float(rate_model_dF),
                "rate_observed_dF": float(rate_obs_dF),
                "delta_rate_dF": float(delta_rate_dF),
                "rate_observed_T": float(rate_observed_T),
                "delta_T_vs_dF": float(delta_T_vs_dF),
            })

            print(
                f"  {dt_prev:.1f} → {dt_curr:.1f}      | {rate_model_dF:13.4f} | {rate_obs_dF:13.4f} | "
                f"{delta_rate_dF:+8.4f} | {rate_observed_T:11.4f} | {delta_T_vs_dF:+9.4f}"
            )
    ...
    # Part 6j: Frobenius convergence rates Rate(d_F) (new in iteration 26)
...
def main() -> int:
    ...
    print("=" * 70)
    print("TTA-UC GKSL Iteration 26: Frobenius-based Composite Model")
    ...
    results: dict = {
        "timestamp": timestamp,
        "iteration": 26,
        "purpose": "Frobenius-based composite model. "
                   "Iteration 25 trace-distance model had 8.7% max error and Rate(T) "
                   "prediction delta=-0.22 at dt=1.0->0.5. Root cause: law of cosines is only "
                   "approximate for L1 (trace distance). Frobenius (L2) model has <1.2% error "
                   "and Rate(d_F) prediction |delta|<0.016. This iteration adds: "
                   "(1) Frobenius coefficient fitting (a_F, b_F), "
                   "(2) Frobenius composite model, "
                   "(3) Frobenius-based Rate(d_F) prediction, "
                   "(4) T/d_F ratio analysis, "
                   "(5) Frobenius convergence rates.",
    ...
    # Write JSON report
    # ===================================================================
    json_path = output_dir / f"iteration26_frobenius_model_{timestamp}.json"
    ...
    # ===================================================================
    md_lines = [
        "# TTA-UC GKSL Iteration 26: Frobenius ベース合成モデル",
        "",
        f"- 実行時刻(UTC): {timestamp}",
        "- iteration 25 からの主要修正:",
        "  - Part 6f 再構築: Frobenius 距離ベースの合成モデル（トレース距離モデルを置換）",
        "  - Part 6h 再構築: Frobenius ベース Rate(d_F) 予測",
        "  - Part 6i（新規）: T/d_F 比率分析",
        "  - Part 6j（新規）: Frobenius 距離の収束次数 Rate(d_F)",
        "  - Parts 6a-6e, 6g: iteration 25 と同一",
        "",
    ]
    ...
    if te.get("composite_fit"):
        ...
        md_lines.extend([
            "### Frobenius ベース合成モデル（iteration 26 再構築）",
            "",
            ...
            "",
            "| dt | d_F_model | d_F_observed | 誤差(Frob) | T_model(iter25) | T_observed | 誤差(Trace) |",
            "|----|-----------|-------------|-----------|-----------------|-----------|-----------|",
        ])
        for k_idx, cd_entry in enumerate(te["convergence_data"]):
            ...
            tT_obs = cd_entry["T_ST_vs_exact"]
            err_T = cf_data["trace_model_relative_errors"][k_idx]
            md_lines.append(
                f"| {cd_entry['dt']:.2f} | {dF_mod:.4e} | {dF_obs:.4e} | {err_F:.4f} | "
                f"{tT_mod:.4e} | {tT_obs:.4e} | {err_T:.4f} |"
            )
        md_lines.append("")
    ...
    if te.get("rate_prediction") and te["rate_prediction"].get("predictions"):
        ...
        md_lines.extend([
            "### Frobenius ベース Rate(d_F) 予測と実測の比較（iteration 26 再構築）",
            "",
            ...
            "",
            "| dt 範囲 | Rate_mod(d_F) | Rate_obs(d_F) | Δ(d_F) | Rate_obs(T) | Δ(T-d_F) |",
            "|---------|-------------|-------------|--------|-----------|----------|",
        ])
        for pred in rp["predictions"]:
            delta_dF_str = f"{pred['delta_rate_dF']:+.4f}" if not np.isnan(pred.get("delta_rate_dF", float("nan"))) else "—"
            delta_TdF_str = f"{pred['delta_T_vs_dF']:+.4f}" if not np.isnan(pred.get("delta_T_vs_dF", float("nan"))) else "—"
            md_lines.append(
                f"| {pred['dt_prev']:.1f} → {pred['dt_curr']:.1f} | "
                f"{pred['rate_model_dF']:.4f} | "
    ...
    if te.get("frobenius_rate_data"):
        ...
        md_lines.extend([
            "### Frobenius 収束次数 Rate(d_F)（iteration 26 追加）",
            "",
            "| dt 範囲 | Rate(d_F) ST-ex | Rate(T) ST-ex | Rate(d_F) Stine | Rate(d_F) Strang |",
            "|---------|----------------|--------------|----------------|-----------------|",
        ])
        for k in range(len(frd.get("frob_ST_vs_exact", []))):
            ...
            md_lines.append(
                ...
                f"{r_dF_s:.4f} | {r_dF_str:.4f} |"
            )
        md_lines.append("")

    md_path = output_dir / f"iteration26_frobenius_model_{timestamp}.md"
    md_path.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
    ...
    if te.get("composite_fit"):
        cf_data = te["composite_fit"]
        print(f"  Test E: Frobenius model r_F=b_F/a_F={cf_data['frobenius_r_F']:.4f}, "
              f"cos(θ_F)={cf_data['frobenius_cos_theta_F_mean']:.4f}, "
              f"max_error={max(cf_data['frobenius_model_relative_errors'])*100:.1f}%")
        print(f"  Test E: Trace model (iter25) max_error={max(cf_data['trace_model_relative_errors'])*100:.1f}% "
              f"(Frobenius is {max(cf_data['trace_model_relative_errors'])/max(cf_data['frobenius_model_relative_errors']):.1f}x more accurate)")
    if te.get("frobenius_angle_data"):
    ...
    if te.get("rate_prediction") and te["rate_prediction"].get("predictions"):
        rp = te["rate_prediction"]
        max_delta_dF = max(abs(p["delta_rate_dF"]) for p in rp["predictions"] if not np.isnan(p.get("delta_rate_dF", float("nan"))))
        print(f"  Test E: Rate(d_F) prediction max |Δ| = {max_delta_dF:.4f}")
        if rp.get("dt_threshold_rate_dF_095") is not None:
