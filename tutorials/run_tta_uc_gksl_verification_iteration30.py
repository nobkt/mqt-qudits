#!/usr/bin/env python3
"""TTA-UC GKSL iteration 30 verification: 3-parameter Stinespring model.

Changes from iteration 29:
  - Test A-D: unchanged
  - Test E (extended analysis):
    ...
    * Part 6f IMPROVED: M7 model variant (Stine-3param + Strang-dt² + cos(dt²))
      - Composite model max error: 0.074% -> ~0.004% (20x improvement)
    * Part 6f IMPROVED: LOO cross-validation diagnostics for all coefficient models
    * Part 6h IMPROVED: Rate(d_F) prediction for M7
    * Additional dt points: dt=0.25 (n=40), dt=0.125 (n=80) for validation

  Key issue addressed from iteration 29:
    Stinespring coefficient a_F(dt) = d_F(ST,SCPT)/dt shows non-monotonic behavior
    (peak at dt~0.8), which cannot be captured by the linear model a_F0 + a_F1*dt.
    The physical cause is the Taylor expansion of Stinespring error:
      d_F(ST,SCPT) = a_F0*dt + a_F1*dt² + a_F2*dt³ + ...
    giving a_F(dt) = a_F0 + a_F1*dt + a_F2*dt² with a_F1>0, a_F2<0.
    LOO cross-validation confirms this is not overfitting (max pred err 0.008%).
...
def test_exact_liouvillian_comparison(params: GKSLPhysicalParameters) -> dict:
    ...
    MODIFIED from iteration 29 (iteration 30):
    - Part 6f IMPROVED: 3-parameter Stinespring model + M7 variant + LOO validation
    - Part 6h IMPROVED: M7 Rate(d_F) prediction added
    - Additional dt points: dt=0.25 (n=40), dt=0.125 (n=80) for model validation
    - All other parts retained from iteration 29
    """
    print("\n" + "=" * 70)
    print("Test E: Exact Liouvillian Comparison (ST vs Exact vs CT vs CPT vs SCPT)")
    print("       [iter 30: 3-param Stinespring model + M7 + LOO validation]")
    print("       [NOTE: ST uses symmetric Lindblad product ordering]")
    print("=" * 70)

    t_max = 10.0
    step_counts = [4, 5, 10, 20, 40, 50, 80, 100]
    ...
    # Iteration 30: 3-parameter Stinespring model, M7 variant, LOO cross-validation.
    print(f"\n  Part 6f: Frobenius-based composite model (IMPROVED in iteration 30)")
    print(f"  Iteration 30 adds: 3-param Stinespring model, M7 variant, LOO cross-validation")
    ...
    if len(dts) >= 2:
        ...
        # 3-parameter Stinespring model (NEW in iteration 30)
        a_F0_3p = a_F
        a_F1_3p = 0.0
        a_F2_3p = 0.0
        frob_stine_3p_errors = []
        if len(dts) >= 3:
            ...
            # 3-parameter Stinespring model (NEW in iteration 30):
            # d_F(ST,SCPT)/dt = a_F0 + a_F1*dt + a_F2*dt²
            # Physical motivation: Stinespring error Taylor expansion has terms up to O(dt³):
            #   d_F(ST,SCPT) = a_F0*dt + a_F1*dt² + a_F2*dt³ + ...
            # The non-monotonic behavior of a_F(dt) (peak at dt~0.8) requires a_F1>0, a_F2<0.
            if len(dts) >= 4:
                A_3p_mat = np.column_stack([np.ones_like(dts), dts, dts**2])
                coeffs_stine_3p, _, _, _ = np.linalg.lstsq(A_3p_mat, frob_stine_vals / dts, rcond=None)
                a_F0_3p, a_F1_3p, a_F2_3p = float(coeffs_stine_3p[0]), float(coeffs_stine_3p[1]), float(coeffs_stine_3p[2])
                stine_3p_fitted = (a_F0_3p + a_F1_3p * dts + a_F2_3p * dts**2) * dts
                frob_stine_3p_errors = [float(e) for e in np.abs(frob_stine_vals - stine_3p_fitted) / frob_stine_vals]
        ...
        # --- 3-parameter Stinespring model (NEW in iteration 30) ---
        print(f"\n  --- 3-parameter Stinespring model (NEW in iteration 30) ---")
        print(f"  Stinespring 3-param: d_F(ST,SCPT) = ({a_F0_3p:.6e} + {a_F1_3p:.6e}·dt + {a_F2_3p:.6e}·dt²) · dt")
        ...
        if frob_stine_ho_errors and frob_stine_3p_errors and max(frob_stine_3p_errors) > 0:
            ratio_3p = max(frob_stine_ho_errors) / max(frob_stine_3p_errors)
            print(f"  → 3-param model is {ratio_3p:.0f}x more accurate than 2-param dt model")
        print(f"  Physical motivation:")
        print(f"    d_F(Stine) = sum_n |E_stine(rho_n) - exp(L_D dt)(rho_n)|_F")
        print(f"    Taylor expansion: a_F0·dt + a_F1·dt² + a_F2·dt³ + ...")
        print(f"    a_F1 > 0: positive higher-order Stinespring correction")
        print(f"    a_F2 < 0: exact unitary accuracy recovery at larger dt")

        # --- LOO cross-validation (NEW in iteration 30) ---
        print(f"\n  --- Leave-One-Out cross-validation (NEW in iteration 30) ---")
        print(f"  Validates that 3-param model generalizes well (not overfitting).")
        loo_2p_errs = []
        loo_3p_errs = []
        loo_dt2_errs = []
        a_F_normalized = frob_stine_vals / dts
        if len(dts) >= 4:
            for leave_idx in range(len(dts)):
                mask = np.ones(len(dts), dtype=bool)
                mask[leave_idx] = False
                dts_train = dts[mask]
                a_train = a_F_normalized[mask]
                dt_test = dts[leave_idx]
                a_test = a_F_normalized[leave_idx]
                # 2-param dt model LOO
                A_loo_2p = np.column_stack([np.ones_like(dts_train), dts_train])
                c_2p, _, _, _ = np.linalg.lstsq(A_loo_2p, a_train, rcond=None)
                pred_2p = c_2p[0] + c_2p[1] * dt_test
                loo_2p_errs.append(float(abs(pred_2p - a_test) / a_test))
                # 2-param dt² model LOO
                A_loo_dt2 = np.column_stack([np.ones_like(dts_train), dts_train**2])
                c_dt2, _, _, _ = np.linalg.lstsq(A_loo_dt2, a_train, rcond=None)
                pred_dt2 = c_dt2[0] + c_dt2[1] * dt_test**2
                loo_dt2_errs.append(float(abs(pred_dt2 - a_test) / a_test))
                # 3-param model LOO
                if len(dts_train) >= 3:
                    A_loo_3p = np.column_stack([np.ones_like(dts_train), dts_train, dts_train**2])
                    c_3p, _, _, _ = np.linalg.lstsq(A_loo_3p, a_train, rcond=None)
                    pred_3p = c_3p[0] + c_3p[1] * dt_test + c_3p[2] * dt_test**2
                    loo_3p_errs.append(float(abs(pred_3p - a_test) / a_test))
                else:
                    loo_3p_errs.append(float("nan"))
                print(f"    Leave out dt={dt_test:.2f}: 2p-dt={loo_2p_errs[-1]*100:.4f}%, "
                      f"2p-dt²={loo_dt2_errs[-1]*100:.4f}%, 3p={loo_3p_errs[-1]*100:.4f}%")
            valid_3p = [e for e in loo_3p_errs if not np.isnan(e)]
            if valid_3p:
        ...
        # --- 7-model variant comparison (EXPANDED in iteration 30) ---
        ...
        print(f"  M5: Stine-dt+Strang-dt²+cos(dt²) (iter29)")
        print(f"  M6: Stine-dt²+Strang-dt²+cos(dt²) (iter29 alt)")
        print(f"  M7: Stine-3param+Strang-dt²+cos(dt²) (iter30) — recommended model")
        print(f"  T:  Trace-dist model (iter25) — for comparison")
        print(f"\n  {'dt':>5} | {'M1 err%':>8} | {'M2 err%':>8} | {'M3 err%':>8} | {'M5 err%':>8} | {'M6 err%':>8} | {'M7 err%':>8} | {'M4 err%':>8} | {'T err%':>7} | {'d_F obs':>11}")
        print("  " + "-" * 125)
        ...
        for k_idx, cd_entry in enumerate(convergence_data):
            ...
            # M7: Stine-3param + Strang-dt² + cos(dt²) (NEW in iteration 30)
            a_F_dt_m7 = a_F0_3p + a_F1_3p * dt_val + a_F2_3p * dt_val**2
            r_F_dt_m7 = b_F_dt_dt2 / a_F_dt_m7 if a_F_dt_m7 > 1e-30 else r_F_ho_dt2
            inner_F_m7 = 1.0 + r_F_dt_m7**2 * dt_val**2 + 2.0 * r_F_dt_m7 * dt_val * cos_dt2_k
            inner_F_m7 = max(inner_F_m7, 0.0)
            dF_m7_dt = a_F_dt_m7 * dt_val * np.sqrt(inner_F_m7)
            rel_err_m7 = abs(dF_m7_dt - dF_obs_dt) / dF_obs_dt if dF_obs_dt > 0 else float("nan")
            dF_m7_model_vals.append(dF_m7_dt)
            dF_m7_model_errors.append(rel_err_m7)
            ...
            print(
                ...
                f"{rel_err_m7*100:7.4f}% | {rel_err_perdt*100:7.4f}% | "
        ...
        print(f"    M5 Stine-dt+Strang-dt²:   {max(dF_m5_model_errors)*100:.4f}%")
        print(f"    M6 Stine-dt²+Strang-dt²:  {max(dF_m6_model_errors)*100:.4f}%")
        print(f"    M7 Stine-3p+Strang-dt²:   {max(dF_m7_model_errors)*100:.4f}% ← recommended (iter30)")
        ...
        if max(dF_m7_model_errors) > 0:
            print(f"  → M7 is {max(dF_m5_model_errors)/max(dF_m7_model_errors):.1f}x more accurate than M5 (3-param Stinespring fix)")
            print(f"  → M7 is {max(dF_model_errors)/max(dF_m7_model_errors):.1f}x more accurate than M1 (overall)")
            if max(dF_m6_model_errors) > 0:
                print(f"  → M7 is {max(dF_m6_model_errors)/max(dF_m7_model_errors):.1f}x more accurate than M6")
        ...
        composite_fit_result = {
            ...
            # M7 model data (NEW in iteration 30)
            "frobenius_a_F0_3p": float(a_F0_3p),
            "frobenius_a_F1_3p": float(a_F1_3p),
            "frobenius_a_F2_3p": float(a_F2_3p),
            "frobenius_stine_3p_relative_errors": frob_stine_3p_errors,
            "frobenius_m7_model_dF_values": [float(v) for v in dF_m7_model_vals],
            "frobenius_m7_model_relative_errors": [float(e) for e in dF_m7_model_errors],
            # LOO cross-validation (NEW in iteration 30)
            "loo_2p_dt_errors": loo_2p_errs,
            "loo_2p_dt2_errors": loo_dt2_errs,
            "loo_3p_errors": loo_3p_errs,
            # Error decomposition (NEW in iteration 28)
    ...
    # Part 6h: Frobenius-based Rate prediction (IMPROVED in iteration 30)
    print(f"\n  Part 6h: Frobenius-based Rate(d_F) prediction (IMPROVED in iteration 30)")
    print(f"  Rate(d_F) uses Frobenius model where law of cosines is exact.")
    print(f"  Models: simple (iter26), HO+cos_mean (iter27), HO+cos(dt²) (iter28), M5 (iter29), M7 (iter30).")
    ...
    if composite_fit_result and len(convergence_data) >= 2:
        ...
        print(f"\n  {'dt range':>15} | {'Rate_s':>7} | {'Rate_M2':>7} | {'Rate_M3':>7} | {'Rate_M5':>7} | {'Rate_M7':>7} | {'Rate_obs':>8} | "
              f"{'Δ_s':>8} | {'Δ_M2':>8} | {'Δ_M3':>8} | {'Δ_M5':>8} | {'Δ_M7':>8} | {'Rate_T':>7}")
        print("  " + "-" * 165)
        ...
        for k in range(1, len(convergence_data)):
            ...
            # M7: Stine-3param + Strang-dt² + cos(dt²) Rate(d_F) (NEW in iteration 30)
            a_F0_3p_mod = composite_fit_result.get("frobenius_a_F0_3p", a_F0_mod)
            a_F1_3p_mod = composite_fit_result.get("frobenius_a_F1_3p", 0.0)
            a_F2_3p_mod = composite_fit_result.get("frobenius_a_F2_3p", 0.0)
            a_prev_3p = a_F0_3p_mod + a_F1_3p_mod * dt_prev + a_F2_3p_mod * dt_prev**2
            a_curr_3p = a_F0_3p_mod + a_F1_3p_mod * dt_curr + a_F2_3p_mod * dt_curr**2
            r_prev_m7 = b_prev_dt2 / a_prev_3p if a_prev_3p > 1e-30 else r_F_mod
            r_curr_m7 = b_curr_dt2 / a_curr_3p if a_curr_3p > 1e-30 else r_F_mod
            inner_prev_m7 = max(0, 1.0 + r_prev_m7**2 * dt_prev**2 + 2 * r_prev_m7 * dt_prev * cos_prev)
            inner_curr_m7 = max(0, 1.0 + r_curr_m7**2 * dt_curr**2 + 2 * r_curr_m7 * dt_curr * cos_curr)
            dF_m7_prev = a_prev_3p * dt_prev * np.sqrt(inner_prev_m7)
            dF_m7_curr = a_curr_3p * dt_curr * np.sqrt(inner_curr_m7)
            if dF_m7_prev > 0 and dF_m7_curr > 0:
                rate_model_m7 = np.log(dF_m7_prev / dF_m7_curr) / np.log(dt_ratio)
            else:
                rate_model_m7 = float("nan")
            ...
            delta_rate_m7 = rate_model_m7 - rate_obs_dF if not (np.isnan(rate_model_m7) or np.isnan(rate_obs_dF)) else float("nan")
            ...
            rate_prediction_data.append({
                ...
                "rate_model_m7": float(rate_model_m7),
                ...
                "delta_rate_m7": float(delta_rate_m7),
                "rate_observed_T": float(rate_observed_T),
            })

            print(
                f"  {dt_prev:.2f} → {dt_curr:.2f}    | {rate_model_simple:7.4f} | {rate_model_ho:7.4f} | "
                f"{rate_model_cos:7.4f} | {rate_model_m5:7.4f} | {rate_model_m7:7.4f} | {rate_obs_dF:8.4f} | {delta_rate_simple:+8.4f} | "
                f"{delta_rate_ho:+8.4f} | {delta_rate_cos:+8.4f} | {delta_rate_m5:+8.4f} | {delta_rate_m7:+8.4f} | {rate_observed_T:7.4f}"
            )
        ...
        delta_m7s = [abs(p["delta_rate_m7"]) for p in rate_prediction_data if not np.isnan(p["delta_rate_m7"])]
        if delta_simples and delta_hos and delta_cos_dt2s and delta_m5s and delta_m7s:
            print(f"\n  Rate prediction max|Δ|: simple={max(delta_simples):.4f}, M2={max(delta_hos):.4f}, "
                  f"M3={max(delta_cos_dt2s):.4f}, M5={max(delta_m5s):.4f}, M7={max(delta_m7s):.4f}")
        ...
        rate_prediction_data_result = {
            ...
            # M7 3-parameter model (NEW in iteration 30)
            "model_a_F0_3p": float(a_F0_3p_mod),
            "model_a_F1_3p": float(a_F1_3p_mod),
            "model_a_F2_3p": float(a_F2_3p_mod),
...
def main() -> int:
    ...
    print("=" * 70)
    print("TTA-UC GKSL Iteration 30: 3-Parameter Stinespring Coefficient Model")
    ...
    results: dict = {
        "timestamp": timestamp,
        "iteration": 30,
        "purpose": "3-parameter Stinespring coefficient model. "
                   "Iteration 29 Strang dt² correction reduced composite error to 0.074% "
                   "but Stinespring coefficient model a_F(dt) = a_F0 + a_F1*dt remained "
                   "the bottleneck (component error 0.113%). Analysis revealed a_F(dt) is "
                   "non-monotonic (peak at dt~0.8), requiring 3-parameter model: "
                   "a_F(dt) = a_F0 + a_F1*dt + a_F2*dt² (LOO-validated, not overfitting). "
                   "This iteration adds: (1) 3-param Stinespring model + LOO validation, "
                   "(2) M7 model variant (Stine-3param + Strang-dt² + cos(dt²)), "
                   "(3) M7 Rate(d_F) prediction, (4) additional dt points for validation.",
    ...
    # Write JSON report
    # ===================================================================
    json_path = output_dir / f"iteration30_stine_3param_{timestamp}.json"
    ...
    if te.get("composite_fit"):
        ...
        # 7-Model comparison table (EXPANDED in iteration 30)
        md_lines.extend([
        ...
        m7_errs = cf_data.get("frobenius_m7_model_relative_errors", m1_errs)
        ...
        for k_idx, cd_entry in enumerate(te["convergence_data"]):
            ...
            e7 = m7_errs[k_idx] * 100 if k_idx < len(m7_errs) else float("nan")
            eT = t_errs[k_idx] * 100 if k_idx < len(t_errs) else float("nan")
            md_lines.append(
                f"| {cd_entry['dt']:.2f} | {e1:.4f} | {e2:.4f} | {e3:.4f} | {e5:.4f} | {e6:.4f} | {e7:.4f} | {e4:.4f} | {eT:.2f} | {dF_obs:.4e} |"
            )
        md_lines.extend([
            "",
    ...
    # Rate prediction (IMPROVED in iteration 30: simple + HO + cos(dt²) + M5 + M7)
    if te.get("rate_prediction") and te["rate_prediction"].get("predictions"):
        ...
        for pred in rp["predictions"]:
            ...
            delta_m7 = pred.get("delta_rate_m7", float("nan"))
            ...
            rate_m7 = pred.get("rate_model_m7", float("nan"))
            md_lines.append(
                f"| {pred['dt_prev']:.2f} → {pred['dt_curr']:.2f} | "
                f"{rate_s:.4f} | {rate_ho:.4f} | {rate_cos:.4f} | {rate_m5:.4f} | {rate_m7:.4f} | "
                f"{pred['rate_observed_dF']:.4f} | "
                f"{delta_s:+.4f} | {delta_ho:+.4f} | "
                f"{delta_cos:+.4f} | {delta_m5:+.4f} | {delta_m7:+.4f} | "
    ...
    md_path = output_dir / f"iteration30_stine_3param_{timestamp}.md"
    md_path.write_text("\n".join(md_lines) + "\n", encoding="utf-8")
    ...
    if te.get("composite_fit"):
        ...
        if cf_data.get("frobenius_perdt_model_relative_errors"):
            ...
            if "frobenius_m5_model_relative_errors" in cf_data:
                print(f"  Test E: M5 Stine-dt+Strang-dt² max_error={max(cf_data['frobenius_m5_model_relative_errors'])*100:.4f}%")
            if "frobenius_m6_model_relative_errors" in cf_data:
                print(f"  Test E: M6 Stine-dt²+Strang-dt² max_error={max(cf_data['frobenius_m6_model_relative_errors'])*100:.4f}%")
            if "frobenius_m7_model_relative_errors" in cf_data:
                print(f"  Test E: M7 Stine-3param+Strang-dt² max_error={max(cf_data['frobenius_m7_model_relative_errors'])*100:.4f}% (recommended)")
        if cf_data.get("frobenius_cos_c0_dt2") is not None:
