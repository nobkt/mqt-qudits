#!/usr/bin/env python3
"""TTA-UC GKSL iteration 41 verification: Boson palindromic Trotter fix + dead _U_stines removal + iteration count update.
...
All iteration-40 checks (1–26) are re-run for regression testing.
New checks 27–32 verify boson simulator correctness and dead-code cleanup.
...
def check_documentation_fixes(notebook_path: str) -> dict:
    """Check that documentation fixes from iteration 41 analysis are applied."""
    with open(notebook_path) as f:
        nb = json.load(f)
    ...
    has_40_cell25 = "40回のイテレーション検証" in cell25_text
    has_old_39_cell25 = "39回のイテレーション検証" in cell25_text
    results["cell25_has_40"] = has_40_cell25
    results["cell25_has_old_39"] = has_old_39_cell25
    results["cell25_ok"] = has_40_cell25 and not has_old_39_cell25

    cell39_text = "".join(nb["cells"][39]["source"])
    has_40_cell39 = "40回の検証イテレーション" in cell39_text
    has_old_39_cell39 = "39回の検証イテレーション" in cell39_text
    results["cell39_has_40"] = has_40_cell39
    results["cell39_has_old_39"] = has_old_39_cell39
    results["cell39_ok"] = has_40_cell39 and not has_old_39_cell39
...
def main() -> None:
    ...
    results: dict = {
        "iteration": 41,
        "timestamp": timestamp,
        "description": "Boson palindromic Trotter fix + dead _U_stines removal + iteration count update (checks 1-32)",
        "params": {
            "t_max": t_max,
    ...
    print("=" * 70)
    print("Iteration 41: Boson palindromic Trotter fix + dead _U_stines removal")
    print("=" * 70)
    ...
    print(f"  Check 7 - Cell 25 has '40回': {doc_fixes['cell25_ok']}")
    print(f"  Check 8 - Cell 39 has '40回': {doc_fixes['cell39_ok']}")
    ...
    # ===================================================================
    # Part 26: NEW — Cell 25/39 "40回" verification (check 31)
    # ===================================================================
    print("\n--- Part 26: Cell 25/39 '40回' verification (check 31) ---")
    ...
    no_39_cell25 = "39回" not in cell25_text
    no_39_cell39 = "39回" not in cell39_text
    has_40_cell25 = "40回" in cell25_text
    has_40_cell39 = "40回" in cell39_text

    cell_40_ok = bool(no_39_cell25 and no_39_cell39 and has_40_cell25 and has_40_cell39)

    checks["check31_cell_40_verification"] = {
        "cell25_no_39": no_39_cell25,
        "cell39_no_39": no_39_cell39,
        "cell25_has_40": has_40_cell25,
        "cell39_has_40": has_40_cell39,
        "ok": cell_40_ok,
    }
    print(f"  Cell 25 no '39回': {no_39_cell25}, has '40回': {has_40_cell25}")
    print(f"  Cell 39 no '39回': {no_39_cell39}, has '40回': {has_40_cell39}")
    print(f"  Check 31 - Cell 25/39 '40回' verification: {cell_40_ok}")
    ...
    # Save results
    # ===================================================================
    json_path = results_dir / f"iteration41_verification_{timestamp}.json"
    with open(json_path, "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False, default=str)
    print(f"\nJSON results saved to: {json_path}")

    md_path = results_dir / f"iteration41_verification_{timestamp}.md"
    md_lines = [
        f"# Iteration 41 検証結果: Boson palindromic Trotter fix + dead _U_stines removal + iteration count update\n",
        f"\n",
        f"- 実行日時: {timestamp}\n",
        f"- 検証対象: 全32チェック（既存26 + 新規6）\n",
        f"\n",
        f"## 収束解析結果（4分子系）\n",
        f"\n",
    ...
    md_lines.extend([
        ...
        f"| 7 | Cell 25 「40回」表記 | {'✓' if doc_fixes['cell25_ok'] else '✗'} |\n",
        f"| 8 | Cell 39 「40回」表記 | {'✓' if doc_fixes['cell39_ok'] else '✗'} |\n",
        ...
        f"| 31 | Cell 25/39 「40回」確認（39回なし） | {'✓' if cell_40_ok else '✗'} |\n",
