...
def check_documentation_fixes(notebook_path: str) -> dict:
    ...
    has_41_cell25 = "41回のイテレーション検証" in cell25_text
    has_old_40_cell25 = "40回のイテレーション検証" in cell25_text
    results["cell25_has_41"] = has_41_cell25
    results["cell25_has_old_40"] = has_old_40_cell25
    results["cell25_ok"] = has_41_cell25 and not has_old_40_cell25

    cell39_text = "".join(nb["cells"][39]["source"])
    has_41_cell39 = "41回の検証イテレーション" in cell39_text
    has_old_40_cell39 = "40回の検証イテレーション" in cell39_text
    results["cell39_has_41"] = has_41_cell39
    results["cell39_has_old_40"] = has_old_40_cell39
    results["cell39_ok"] = has_41_cell39 and not has_old_40_cell39
...
def main() -> None:
    ...
    results: dict = {
        "iteration": 42,
        "timestamp": timestamp,
        "description": "Iteration count update (41回) + regression tests (checks 1-33)",
        "params": {
            "t_max": t_max,
    ...
    print("=" * 70)
    print("Iteration 42: Iteration count update (41回) + regression tests")
    print("=" * 70)
    ...
    print(f"  Check 7 - Cell 25 has '41回': {doc_fixes['cell25_ok']}")
    print(f"  Check 8 - Cell 39 has '41回': {doc_fixes['cell39_ok']}")
    ...
    # ===================================================================
    # Part 26: Cell 25/39 "41回" verification (check 31)
    # ===================================================================
    print("\n--- Part 26: Cell 25/39 '41回' verification (check 31) ---")
    ...
    no_40_cell25 = "40回" not in cell25_text
    no_40_cell39 = "40回" not in cell39_text
    has_41_cell25 = "41回" in cell25_text
    has_41_cell39 = "41回" in cell39_text

    cell_41_ok = bool(no_40_cell25 and no_40_cell39 and has_41_cell25 and has_41_cell39)

    checks["check31_cell_41_verification"] = {
        "cell25_no_40": no_40_cell25,
        "cell39_no_40": no_40_cell39,
        "cell25_has_41": has_41_cell25,
        "cell39_has_41": has_41_cell39,
        "ok": cell_41_ok,
    }
    print(f"  Cell 25 no '40回': {no_40_cell25}, has '41回': {has_41_cell25}")
    print(f"  Cell 39 no '40回': {no_40_cell39}, has '41回': {has_41_cell39}")
    print(f"  Check 31 - Cell 25/39 '41回' verification: {cell_41_ok}")
    ...
    # Save results
    # ===================================================================
    json_path = results_dir / f"iteration42_verification_{timestamp}.json"
    with open(json_path, "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False, default=str)
    print(f"\nJSON results saved to: {json_path}")

    md_path = results_dir / f"iteration42_verification_{timestamp}.md"
    md_lines = [
        f"# Iteration 42 検証結果: Iteration count update (41回) + regression tests\n",
        f"\n",
        f"- 実行日時: {timestamp}\n",
        f"- 検証対象: 全33チェック（既存32 + 新規1）\n",
        f"\n",
        f"## 収束解析結果（4分子系）\n",
        f"\n",
    ...
    md_lines.extend([
        ...
        f"| 7 | Cell 25 「41回」表記 | {'✓' if doc_fixes['cell25_ok'] else '✗'} |\n",
        f"| 8 | Cell 39 「41回」表記 | {'✓' if doc_fixes['cell39_ok'] else '✗'} |\n",
        ...
        f"| 31 | Cell 25/39 「41回」確認（40回なし） | {'✓' if cell_41_ok else '✗'} |\n",
