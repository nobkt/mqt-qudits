#!/usr/bin/env python3
"""TTA-UC GKSL iteration 43 verification: Iteration count update (42回).

This iteration verifies the updates from iteration 42:

  - Notebook Cell 25/39 verified to contain "42回" (updated from "41回")

All iteration-42 checks (1–33) are re-run for regression testing.
Check 34 (new) verifies Stinespring single-channel approximation error O(dt²).
...
def check_documentation_fixes(notebook_path: str) -> dict:
    ...
    has_42_cell25 = "42回のイテレーション検証" in cell25_text
    has_old_41_cell25 = "41回のイテレーション検証" in cell25_text
    results["cell25_has_42"] = has_42_cell25
    results["cell25_has_old_41"] = has_old_41_cell25
    results["cell25_ok"] = has_42_cell25 and not has_old_41_cell25

    cell39_text = "".join(nb["cells"][39]["source"])
    has_42_cell39 = "42回の検証イテレーション" in cell39_text
    has_old_41_cell39 = "41回の検証イテレーション" in cell39_text
    results["cell39_has_42"] = has_42_cell39
    results["cell39_has_old_41"] = has_old_41_cell39
    results["cell39_ok"] = has_42_cell39 and not has_old_41_cell39
...
def main() -> None:
    ...
    results: dict = {
        "iteration": 43,
        "timestamp": timestamp,
        "description": "Iteration count update (42回) + regression tests (checks 1-34)",
        "params": {
            "t_max": t_max,
    ...
    print("=" * 70)
    print("Iteration 43: Iteration count update (42回) + regression tests")
    print("=" * 70)
    ...
    print(f"  Check 7 - Cell 25 has '42回': {doc_fixes['cell25_ok']}")
    print(f"  Check 8 - Cell 39 has '42回': {doc_fixes['cell39_ok']}")
    ...
    # ===================================================================
    # Part 26: Cell 25/39 "42回" verification (check 31)
    # ===================================================================
    print("\n--- Part 26: Cell 25/39 '42回' verification (check 31) ---")
    ...
    no_41_cell25 = "41回" not in cell25_text
    no_41_cell39 = "41回" not in cell39_text
    has_42_cell25 = "42回" in cell25_text
    has_42_cell39 = "42回" in cell39_text

    cell_42_ok = bool(no_41_cell25 and no_41_cell39 and has_42_cell25 and has_42_cell39)

    checks["check31_cell_42_verification"] = {
        "cell25_no_41": no_41_cell25,
        "cell39_no_41": no_41_cell39,
        "cell25_has_42": has_42_cell25,
        "cell39_has_42": has_42_cell39,
        "ok": cell_42_ok,
    }
    print(f"  Cell 25 no '41回': {no_41_cell25}, has '42回': {has_42_cell25}")
    print(f"  Cell 39 no '41回': {no_41_cell39}, has '42回': {has_42_cell39}")
    print(f"  Check 31 - Cell 25/39 '42回' verification: {cell_42_ok}")
    ...
    # Save results
    # ===================================================================
    json_path = results_dir / f"iteration43_verification_{timestamp}.json"
    with open(json_path, "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False, default=str)
    print(f"\nJSON results saved to: {json_path}")

    md_path = results_dir / f"iteration43_verification_{timestamp}.md"
    md_lines = [
        f"# Iteration 43 検証結果: Iteration count update (42回) + regression tests\n",
        f"\n",
        f"- 実行日時: {timestamp}\n",
    ...
    md_lines.extend([
        ...
        f"| 7 | Cell 25 「42回」表記 | {'✓' if doc_fixes['cell25_ok'] else '✗'} |\n",
        f"| 8 | Cell 39 「42回」表記 | {'✓' if doc_fixes['cell39_ok'] else '✗'} |\n",
        ...
        f"| 31 | Cell 25/39 「42回」確認（41回なし） | {'✓' if cell_42_ok else '✗'} |\n",
