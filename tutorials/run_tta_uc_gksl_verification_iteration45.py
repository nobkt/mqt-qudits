...
def check_documentation_fixes(notebook_path: str) -> dict:
    ...
    has_44_cell25 = "44回のイテレーション検証" in cell25_text
    has_old_43_cell25 = "43回のイテレーション検証" in cell25_text
    results["cell25_has_44"] = has_44_cell25
    results["cell25_has_old_43"] = has_old_43_cell25
    results["cell25_ok"] = has_44_cell25 and not has_old_43_cell25

    cell39_text = "".join(nb["cells"][39]["source"])
    has_44_cell39 = "44回の検証イテレーション" in cell39_text
    has_old_43_cell39 = "43回の検証イテレーション" in cell39_text
    results["cell39_has_44"] = has_44_cell39
    results["cell39_has_old_43"] = has_old_43_cell39
    results["cell39_ok"] = has_44_cell39 and not has_old_43_cell39
...
def main() -> None:
    ...
    results: dict = {
        "iteration": 45,
        "timestamp": timestamp,
        "description": "Iteration count update (44回) + regression tests (checks 1-34)",
        "params": {
            "t_max": t_max,
    ...
    print("=" * 70)
    print("Iteration 45: Iteration count update (44回) + regression tests")
    print("=" * 70)
    ...
    print(f"  Check 7 - Cell 25 has '44回': {doc_fixes['cell25_ok']}")
    print(f"  Check 8 - Cell 39 has '44回': {doc_fixes['cell39_ok']}")
    ...
    # ===================================================================
    # Part 26: Cell 25/39 "44回" verification (check 31)
    # ===================================================================
    print("\n--- Part 26: Cell 25/39 '44回' verification (check 31) ---")
    ...
    no_43_cell25 = "43回" not in cell25_text
    no_43_cell39 = "43回" not in cell39_text
    has_44_cell25 = "44回" in cell25_text
    has_44_cell39 = "44回" in cell39_text

    cell_44_ok = bool(no_43_cell25 and no_43_cell39 and has_44_cell25 and has_44_cell39)

    checks["check31_cell_44_verification"] = {
        "cell25_no_43": no_43_cell25,
        "cell39_no_43": no_43_cell39,
        "cell25_has_44": has_44_cell25,
        "cell39_has_44": has_44_cell39,
        "ok": cell_44_ok,
    }
    print(f"  Cell 25 no '43回': {no_43_cell25}, has '44回': {has_44_cell25}")
    print(f"  Cell 39 no '43回': {no_43_cell39}, has '44回': {has_44_cell39}")
    print(f"  Check 31 - Cell 25/39 '44回' verification: {cell_44_ok}")
    ...
    # Save results
    # ===================================================================
    json_path = results_dir / f"iteration45_verification_{timestamp}.json"
    with open(json_path, "w") as f:
        json.dump(results, f, indent=2, ensure_ascii=False, default=str)
    print(f"\nJSON results saved to: {json_path}")

    md_path = results_dir / f"iteration45_verification_{timestamp}.md"
    md_lines = [
        f"# Iteration 45 検証結果: Iteration count update (44回) + regression tests\n",
        f"\n",
        f"- 実行日時: {timestamp}\n",
    ...
    md_lines.extend([
        ...
        f"| 7 | Cell 25 「44回」表記 | {'✓' if doc_fixes['cell25_ok'] else '✗'} |\n",
        f"| 8 | Cell 39 「44回」表記 | {'✓' if doc_fixes['cell39_ok'] else '✗'} |\n",
        ...
        f"| 31 | Cell 25/39 「44回」確認（43回なし） | {'✓' if cell_44_ok else '✗'} |\n",
